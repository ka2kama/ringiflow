...
impl AuditLogRepository for DynamoDbAuditLogRepository {
   async fn record(&self, log: &AuditLog) -> Result<(), InfraError> {
      ...
      if let Some(detail) = &log.detail {
         item.insert(
            "detail".to_string(),
            AttributeValue::S(serde_json::to_string(detail).map_err(InfraError::Serialization)?),
         );
      }
   ...
   async fn find_by_tenant(
      ...
      match (&filter.from, &filter.to) {
         (Some(from), Some(to)) => {
            ...
            expr_attr_values.insert(":sk_from".to_string(), AttributeValue::S(from.to_rfc3339()));
      ...
      if let Some(cursor_str) = cursor {
         ...
         let key_strings: HashMap<String, String> = serde_json::from_slice(&decoded)
            .map_err(|e| InfraError::DynamoDb(format!("カーソルのデシリアライズに失敗: {e}")))?;
