...
/// 詳細: [ADR-054](../../docs/70_ADR/054_型安全ステートマシンパターンの標準化.md), [エンティティ影響マップ](../../docs/40_詳細設計書/エンティティ影響マップ/WorkflowStep.md) INV-S2〜S4
...
/// 担当者への割り当てと判断結果を保持する。
///
/// [ADR-054](../../docs/70_ADR/054_型安全ステートマシンパターンの標準化.md) Pattern A: 共通フィールドを外側に、状態固有フィールドを `state` enum に分離。
#[derive(Debug, Clone, PartialEq, Eq)]
