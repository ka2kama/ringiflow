...
mod tests {
   ...
   use super::super::test_helpers::{setup_two_step_approval, single_approval_definition_json};
   use crate::{
      error::CoreError,
      usecase::workflow::{ApproveRejectInput, WorkflowUseCaseImpl, WorkflowWithSteps},
   };
   ...
   async fn test_reject_step_最終ステップ_インスタンスがrejectedになる() {
      ...
      definition_repo.add_definition(definition);
      instance_repo.insert(&instance_after_first).await.unwrap();
      step_repo
         .insert(&step1_completed, &tenant_id)
         .await
         .unwrap();
      step_repo.insert(&step2_active, &tenant_id).await.unwrap();
