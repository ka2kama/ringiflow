...
TDD サイクルの Refactor ステップで適用する。Refactor は1回の改善で終わりではなく、このチェックリストの指摘がゼロになるまで繰り返す。Phase 完了後にも改めて全体を通して検証する。
...
| # | 観点 | 確認内容 |
|---|------|---------|
| 1 | Phase 間データフロー整合 | 下位レイヤーの出力（DTO、レスポンス型）が上位レイヤーの期待する入力と一致しているか。フィールドの追加・変更が全レイヤーに伝播しているか |
| 2 | 仕様との端到端突合 | OpenAPI 仕様 → Core Service → BFF → フロントエンドデコーダーのデータが一貫しているか。Silent Failure（オプショナルフィールドでのフォールバック）が潜んでいないか |
| 3 | 完了基準の E2E 検証 | Issue の完了基準（特に E2E 基準「ユーザーが UI から操作を完了できる」）がすべて達成されているか |
...
| # | 観点 | 判定 | 確認内容 |
|---|------|------|---------|
| 1 | 網羅性 | OK | [具体的に何を確認したか] |
| 2 | 曖昧さ排除 | OK | [具体的に何を確認したか] |
| 3 | 設計判断の完結性 | OK | [具体的に何を確認したか] |
| 4 | スコープ境界 | OK | [具体的に何を確認したか] |
| 5 | 技術的前提 | OK | [具体的に何を確認したか] |
| 6 | 既存ドキュメント整合 | OK | [具体的に何を確認したか] |
```

目的: 行動規範（「検証すること」）ではなく成果物要件（「検証結果が記載されていること」）にすることで、検証の欠落を構造的に防ぐ。
