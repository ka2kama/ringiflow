...
TDD 開発フローの AI ディレクティブ版。教育的解説・参考資料は [手順書](../../docs/60_手順書/04_開発フロー/02_TDD開発フロー.md) を参照。
...
| 型の活用 | 不正な状態や操作を型で防げていないか？ Newtype（[ADR-016](../../docs/70_ADR/016_プリミティブ型のNewtype化方針.md)）/ 型安全ステートマシン（[ADR-054](../../docs/70_ADR/054_型安全ステートマシンパターンの標準化.md)）/ 構造的強制（[ADR-051](../../docs/70_ADR/051_トランザクションコンテキストによる構造的強制.md)）/ 境界での型変換 / 不変条件の保護 |

発見があった場合のみ、セッションログの判断ログに記録する。

#### UI/UX レンズ（フロントエンド変更時）

→ デザインガイドライン: [`docs/40_詳細設計書/13_デザインガイドライン.md`](../../docs/40_詳細設計書/13_デザインガイドライン.md)
...
→ テストピラミッドの概念整理: [ナレッジベース: テストピラミッド](../../docs/80_ナレッジベース/methodology/テストピラミッド.md)

| テスト層 | 検証対象 | 該当条件 |
|---------|---------|---------|
| ユニットテスト | ドメインロジック、ユースケース | 常に |
| ハンドラテスト | HTTP ハンドラの入出力、認可 | HTTP ハンドラがある場合 |
| API テスト（Hurl） | 実 DB を含むエンドポイント結合 | 公開 API エンドポイントがある場合。フロントエンドが既存 API を呼び出す場合は、リクエスト/レスポンスの契約（フィールド名・型）が既存テストで検証済みか確認し、未検証なら追加する |
...
テストリスト作成後、TDD の Red に入る前に、テスト設計技法に基づく導出観点でテストケースの抜け漏れを検出する。設計ブラッシュアップループ（[zoom-rhythm.md](zoom-rhythm.md#設計ブラッシュアップループ)）とは独立したループ。

```mermaid
flowchart LR
    作成["テストリスト初版作成"] --> レビュー["導出観点でレビュー"]
    レビュー --> 判定{ギャップ}
    判定 -->|あり| 追加["テストケース追加"] --> レビュー
    判定 -->|ゼロ| Red["TDD の Red へ"]
```

#### 導出観点

各操作パスのテストケースに対して、以下の観点を適用する。対象の性質に応じて該当する観点を選択する（全観点を毎回適用する必要はない）。

| 観点 | 問い | 該当する場面 | 源泉 |
|------|------|------------|------|
| 入力の分類 | 各入力に対して有効/無効の同値クラスを検討したか | 値を受け取る処理 | 同値分割 |
| 境界値 | 同値クラスの境界をテストしているか | 数値・文字列長・日付等の範囲がある入力 | 境界値分析 |
| 条件の組み合わせ | 複数条件が絡む場合、組み合わせを検討したか | 複合条件のビジネスルール | デシジョンテーブル |
| 状態遷移 | 状態を持つ対象で、遷移と無効遷移を検討したか | ライフサイクルを持つエンティティ | 状態遷移テスト |
| システム境界の契約 | 操作パスが通過するシステム境界でのデータ変換・契約を検証しているか | コンポーネント間連携（API、DB、外部サービス） | Weyuker の反合成性、契約テスト |
| エラー系の網羅 | 各操作パスの失敗モード（バリデーション、権限、リソース不在等）を検討したか | 外部入力・外部依存がある処理 | エラー推測 |

#### 既知手法との関連

→ 詳細: [KB: 独自フレームワークと既知手法の対応](../../docs/80_ナレッジベース/methodology/独自フレームワークと既知手法の対応.md)

- ISTQB のテスト設計技法（ブラックボックス技法）を「操作パスに対する導出観点」として再構成
- Weyuker の反合成性公理: 各コンポーネントに十分なテストセットの合併が合成されたプログラムに十分とは限らない → 「システム境界の契約」観点の理論的根拠
...
→ E2E テストの詳細: [Playwright ナレッジベース](../../docs/80_ナレッジベース/devtools/Playwright.md)
