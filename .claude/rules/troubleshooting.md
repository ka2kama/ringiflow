...
認知バイアスへの対抗:
- 可用性ヒューリスティック: 手元で確認できるもの（コード、ログ、DB）を優先しがち
- 内部帰属バイアス: 原因を自システム内部に求めがち
- ツール法則: Read/Grep/DB クエリが得意だと、すべてをそれで解決しようとする

既知手法との関連:
- 製造業の 5M（Man, Machine, Material, Method, Environment）— 原因の探索カテゴリを構造化する手法
- Ishikawa Diagram（特性要因図）の「環境」カテゴリ — 外部要因を明示的に探索空間に含める
- SRE の "correlate with changes"（変更があったものを疑え）— カテゴリ 1-4 の着眼点
...
→ 詳細: [KB: 独自フレームワークと既知手法の対応](../../docs/06_ナレッジベース/methodology/独自フレームワークと既知手法の対応.md)
