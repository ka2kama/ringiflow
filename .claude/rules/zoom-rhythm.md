...
各ループで何を発見し、どう解決したかを記録する。この記録は設計の思考過程を可視化し、後続の判断の根拠となる。
...
| ループ | きっかけ | 調査内容 | 結果 |
|-------|---------|---------|------|
| 1回目 | 初版完成 → 不確実性の洗い出し | [何を調査したか] | [何が変わったか] |
| 2回目 | 改訂後 → 残りのギャップ探索 | [何を調査したか] | [何が変わったか] |
```

#### 実装中の変化との関係

設計段階で徹底的に詰めても、実装中に変化は生じる。しかし:

- 設計を詰めた場合: 変化は「調整」レベル（小さな修正）
- 設計が甘い場合: 変化は「手戻り」レベル（大きな再設計）

設計ブラッシュアップは、実装中の変化を「調整」に留めるための投資。
...
実装中の俯瞰は、TDD の Refactor ステップに**設計原則レンズ**として組み込まれている。「視点を上げろ」という抽象的な行動規範ではなく、具体的な設計原則の問いに答えることで実現する。

→ 具体的な問いとトリガー: [TDD 開発フロー > Refactor: 設計を改善する](../../docs/04_手順書/04_開発フロー/02_TDD開発フロー.md#-refactor-設計を改善する)

| タイミング | レンズ | 目的 |
|-----------|--------|------|
| 毎 Refactor | Simple Design（意図・重複・最小性） | 書いた直後が最も気づきやすい |
| モジュール/関数の完成時 | SRP・DIP・型の活用 | 単位が見えてから評価できる |
| Phase 完了時 | 全レンズ + [品質チェックリスト](../../docs/04_手順書/04_開発フロー/01_Issue駆動開発.md#62-品質チェックリスト) | 区切りでの全体俯瞰 |

Refactor で品質を**作り込み**、品質チェックリスト（最後の砦）で**漏れを防ぐ**。

発見があった場合のみ、セッションログの判断ログに記録する。

改善の経緯: [往復のリズムが機能しない構造的原因](../../prompts/improvements/2026-02/2026-02-08_往復のリズムが機能しない構造的原因.md)
...
**まず理想状態（To-Be）を定義し、現状（As-Is）を理想に近づける。** これが「理想駆動」の本質。[ベストプラクティス起点](../../CLAUDE.md#共通アプローチ-ベストプラクティス起点)の方法論的実現でもある。
...
TDD サイクルの Refactor ステップに組み込まれた [設計原則レンズ](../../docs/04_手順書/04_開発フロー/02_TDD開発フロー.md#設計原則レンズ) が、実装中の俯瞰を具体化する。各サイクルで設計原則の問いに答え、改善機会を探す。
...
品質の追求は、開発フローの中に二つのチェックポイントを設けることで担保する。それぞれが「マイナス→ゼロ」と「ゼロ→プラス」の両面を持つ。

```mermaid
flowchart LR
    A["設計"] --> B["最初の砦<br/>ブラッシュアップ"]
    B --> C["実装<br/>設計原則レンズ"]
    C --> D["最後の砦<br/>Ready for Review"]
    D --> E["レビュー"]
```

| | 最初の砦 | 最後の砦 |
|---|---------|---------|
| タイミング | 設計段階（実装前） | Ready for Review（実装後） |
| プロセス | [設計ブラッシュアップループ](#設計ブラッシュアップループ) | [Ready for Review](../../docs/04_手順書/04_開発フロー/01_Issue駆動開発.md#6-ready-for-review) |
| マイナス→ゼロ | ギャップ発見（欠陥の発見） | 整合性チェックリスト |
| ゼロ→プラス | ギャップ発見（品質の向上） | 品質向上の最終確認 |

最初の砦で品質を作り込み、最後の砦で漏れを防ぐ。最後の砦だけに頼ると実装後の手戻りが大きくなり、最初の砦だけに頼ると実装中に生じた品質低下を見逃す。
...
計画ファイル（plan mode で作成するファイル）には、末尾に以下の2つのセクションを記載する。記載されていない計画は完成とみなさない。
...
| フレームワークの要素 | 対応する既知手法 |
|---------------------|-----------------|
| 俯瞰と局所の往復 | OODA ループ（Boyd） |
| To-Be の更新（前提自体の見直し） | ダブルループ学習（Argyris & Schön） |
| 実践中の視点切り替え | リフレクティブ・プラクティス（Schön） |

独自の追加要素:
- 二次元ギャップ: マイナス→ゼロ（欠陥除去）とゼロ→プラス（品質向上）の区別
- 収束の定義: To-Be = As-Is でギャップゼロという明確な完了基準
- 二つの砦: 設計段階と Ready for Review の二段構えによる品質保証
- 設計原則レンズ: 実装中の俯瞰を SOLID / Simple Design の具体的な問いとして操作化
- トリガーの形式知化: 「いつ視点を上げるか」の明示的な表
...
- 改善記録:
  - [往復のリズムが機能しない構造的原因](../../prompts/improvements/2026-02/2026-02-08_往復のリズムが機能しない構造的原因.md)
