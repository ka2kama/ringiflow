...
実装前に確認すべき型・パターン・ライブラリ API・リファクタリング操作の影響を事前に洗い出す。
...
品質チェックリスト（6.2）のカテゴリ別にチェックボックス + 確認内容 1 行で記載する。該当しないカテゴリは `[x] N/A（理由）`。全行が `[x]` であること。

**実装の場合:**
```markdown
## Self-review

- [x] 設計・ドキュメント: 詳細設計書更新済み、ADR該当なし
- [x] Issue との整合: 完了基準・実装計画すべて [x] 更新済み
- [x] テスト: テストリスト全項目実装、全層カバー
- [x] コード品質（マイナス→ゼロ）: レイヤー違反なし、既存パターン準拠、OpenAPI同期確認済み
- [x] 品質向上（ゼロ→プラス）: 不要な中間層なし、責務単一
- [x] UI/UX: N/A（バックエンドのみ）
- [x] 横断検証: N/A（単一Phase）
- [x] `just check-all` pass: 全テスト通過
```
...
- [x] 参照の網羅性: 関連ファイルの参照をすべて更新
- [x] 用語の統一: 新旧用語が混在していないことを確認
- [x] 意図の明確さ: 変更の目的が読み手に伝わることを確認
- [x] `just check-all` pass: 全テスト通過
```
