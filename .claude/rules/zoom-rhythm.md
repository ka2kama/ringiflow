...
| 状態網羅漏れ | UI で RemoteData の全バリアント（NotAsked/Loading/Failure/Success）と空データの一部が未処理のもの |
...
Phase 完了後にも改めて全体を通して検証する。→ 品質チェックリスト: [Issue駆動開発 > 品質チェックリスト](../../docs/04_手順書/04_開発フロー/01_Issue駆動開発.md#62-品質チェックリスト)
...
→ 品質チェックリスト: [Issue駆動開発 > 品質チェックリスト](../../docs/04_手順書/04_開発フロー/01_Issue駆動開発.md#62-品質チェックリスト)
...
設計段階と品質ゲートの二段構えで品質を担保する: 設計 → 最初の砦（ブラッシュアップ）→ 実装（設計原則レンズ）→ 最後の砦（品質ゲート）→ レビュー
...
各 Phase に「確認事項」セクションを記載する。実装前に確認すべき型・パターン・ライブラリ API・リファクタリング操作の影響を事前に洗い出す。
...
- [ ] リファクタリング: [分割対象ファイルの重複パターン] → [対象ファイル]
```
...
- [x] リファクタリング: decision.rs の重複パターン → approve/reject/request_changes に共通構造あり、共通化を先行
```
