...
初版を書いただけでは設計は完成しない。**「この設計が現時点での最適解だ」と胸を張れるレベル**まで、繰り返しギャップを洗い出して解決する。ループ数に上限はない。むしろ複数回のループを経ることを推奨する。
...
| 観点 | 探すもの | 例 |
|------|---------|-----|
| 未定義 | 参照されているが定義されていないもの | パーサー関数名のみ記載、実装未定義 |
| 曖昧 | 複数の解釈が可能なもの | 「フィルタを適用する」の具体的な適用順序 |
| 競合・エッジケース | 組み合わせの挙動が未定義のもの | 2つのフィルタが同時に指定された場合 |
| 状態漏れ | UI の状態遷移が不完全なもの | フィルタ解除方法、ドロップダウンの表示状態 |
| アーキテクチャ矛盾 | 純粋関数 vs 副作用の境界が曖昧なもの | 純粋関数が副作用を必要とするフロー |
| 車輪の再発明 | 標準ライブラリに代替手段があるもの | 手動パース vs 標準パーサーモジュール |
