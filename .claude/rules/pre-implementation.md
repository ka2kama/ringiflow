...
- API 境界を跨ぐ型のフィールド突合（フロントエンドのエンコーダ/デコーダを書く場合、対応するバックエンドのリクエスト/レスポンス型と OpenAPI 仕様のフィールド名を Read で確認する）
...
新しい型（Elm の `type alias Model`、Rust の `struct`）を定義する際、以下に該当する場合は型安全ステートマシン（[ADR-054](../../docs/70_ADR/054_型安全ステートマシンパターンの標準化.md)）を適用する。

判定基準:
- 状態フィールド（`RemoteData`、`Status` enum 等）と、その状態でのみ有効なフィールドが同一の型に存在するか？
- `Maybe` / `Option` が「この状態では存在しない」の意味を含んでいないか？

該当する場合、状態ごとに異なる型を定義し、各状態で有効なフィールドだけを持たせる。
...
3. 計画ファイルの確認事項に「分割対象ファイルの重複パターン」を記載する
...
→ 実施タイミング: [TDD 開発フロー > 確認事項の実施](dev-flow-tdd.md#確認事項の実施)
...
改善の経緯: [計画ファイルの確認事項チェック漏れの頻発](../../process/improvements/2026-02/2026-02-20_2100_計画ファイルの確認事項チェック漏れの頻発.md)
