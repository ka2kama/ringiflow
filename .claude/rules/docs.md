...
| `process/improvements/**` | `YYYY-MM-DD_HHMM_<トピック>.md` |
...
連番体系の例: 10_要件定義書、60_手順書
コレクション型の例: 70_ADR、80_ナレッジベース、90_実装解説
...
注: 30_基本設計書、40_詳細設計書は `00_` スロットが既存ファイル（`00_アーキテクチャ概要.md`、`00_実装ロードマップ.md`）に占有されているため、`00_はじめに.md` をエントリファイルとして追加する場合はリネームが必要。50_テストはディレクトリ構造が未成熟のため保留。
...
改善の経緯: [命名規則を読んだが従わなかった](../../process/improvements/2026-01/2026-01-29_1307_命名規則を読んだが従わなかった.md)
...
| 3 | サニタイズ | 認証情報・PII・内部 URL が除去されているか。改善記録ではユーザー発言の直接引用を避ける。セッションログではユーザー発言の内容を保持する（口調の整形は可、公開に不適切な発言を除き省略は不可） |
...
改善の経緯:
- [会話コンテキストのドキュメント混入](../../process/improvements/2026-02/2026-02-06_2330_会話コンテキストのドキュメント混入.md)
- [改善記録にユーザー発言を直接引用した](../../process/improvements/2026-02/2026-02-09_2122_改善記録にユーザー発言を直接引用した.md)
- [ルールファイル追記時の文体規約違反](../../process/improvements/2026-02/2026-02-09_2117_ルールファイル追記時の文体規約違反.md)
- [Test plan チェックボックスの形骸化](../../process/improvements/2026-02/2026-02-14_2130_Test-planチェックボックスの形骸化.md)

## 設計書の実装状態マーカー

設計書（`docs/30_基本設計書/`、`docs/40_詳細設計書/`）において、未実装または部分的に実装済みのセクションには blockquote 形式で実装状態を明示する。

### 基本ルール

- マーカーなし = 実装済み（デフォルト）
- 未実装・部分的にのみマーカーを付与する（例外マーキング方式）
- Phase 番号は [実装ロードマップ](../../docs/40_詳細設計書/00_実装ロードマップ.md) に準拠する
