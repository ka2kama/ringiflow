...
<!-- 改善: Issue #1086 PR 確認項目の手動確認を自動テスト化するプロセスの導入 -->
- 手動確認項目（開発サーバーでの動作確認等）を含める場合は、自動テスト化できない理由を併記する。理由なしの手動確認項目は品質ゲートでブロックする

**禁止:** Draft なしで PR を作成すること

### PR 本文への Mermaid 図の埋め込み

計画ファイルに作成した Mermaid 図のうち、レビューに有用なものを PR 本文の概要セクションに埋め込む。

| 適用条件 | 埋め込む図 |
|---------|-----------|
| 多層サービス連携 | シーケンス図（コンポーネント間のリクエストフロー） |
| 状態遷移の変更 | 状態遷移図（変更前後の差分が分かるもの） |
| アーキテクチャ変更 | フロー図（変更後の構成） |
| 複数 Phase の PR | フロー図（Phase 間の依存関係） |

単一レイヤーの変更や計画ファイルに図がない場合は不要。

→ 図の適用条件と種類の詳細: [zoom-rhythm.md > Mermaid 図の活用](zoom-rhythm.md#mermaid-図の活用)
...
計画ファイルを3層構造（仕様整理 → 設計 → ブラッシュアップ状態）で作成する。→ [zoom-rhythm.md > 計画ファイルへの記載](zoom-rhythm.md#計画ファイルへの記載必須)

1. 仕様整理: スコープ（対象・対象外）を定義し、完了基準から操作パスを導出する
2. 設計: 設計判断を記録し、Phase 分割・確認事項・テストリストを作成する
3. ブラッシュアップ: ギャップ発見の観点で体系的に探索し、進行状態を逐次永続化する

Phase 分割基準: 依存順（下位 → 上位レイヤー）。フルスタック機能の場合、既存 API の拡張も Phase に含める。

計画ファイル作成後、Issue 本文の実装計画セクションに計画ファイルパスを記載する（Issue → 計画ファイルの導線）。計画ファイル側のコンテキストセクションに Issue 番号を記載する（計画ファイル → Issue の導線）。
...
- [ ] PR 確認項目の全項目がチェック済みか
- [ ] PR 確認項目に手動確認（開発サーバーでの動作確認等）が含まれる場合、自動テスト化を検討したか。自動テスト化できない場合は理由を PR 本文に記載
