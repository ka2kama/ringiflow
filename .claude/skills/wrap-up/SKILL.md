...
以下のチェックリストに基づき判定する。各項目に 🟢（作成推奨）/ ⚪（該当なし）を付け、推奨理由を添える。

| ドキュメント | 作成条件 | 配置先 |
|-------------|---------|--------|
| セッションログ | コード変更または設計判断があった | `prompts/runs/YYYY-MM/` |
| ADR | 技術選定、実装方針の選択・見送りがあった | `docs/70_ADR/` |
| ナレッジベース | 新しいツール・パターン導入、技術解説が必要 | `docs/80_ナレッジベース/` |
| 実装解説 | 設計判断を伴う実装があった。`/explain` の実行を推奨 | `docs/90_実装解説/` |
...
判定結果をユーザーに提示し、🟢 のドキュメントは確認を待たずに Step 3 で自動作成する。修正・追加があればオーナーが事後に指示する。
...
テンプレート: `docs/70_ADR/template.md`

1. 次の ADR 番号を確認:
   ```bash
   ls docs/70_ADR/*.md | sort | tail -3
   ```
2. テンプレートに従って作成
...
`/explain` で生成する。wrap-up では生成せず、`/explain` を自動実行する。
...
4. 対策セクションで確認: 問題の性質が記載されていること、配置先が routing table と一致していること、対策形式が検討済みであること
5. 分類セクションで照合手順を実施（カテゴリ 6 種、失敗タイプ 3 種、問題の性質 3 種）
