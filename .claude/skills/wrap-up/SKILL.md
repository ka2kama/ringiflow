...
- 非自明な問題解決があったか
- AI 運用上の問題があったか
- 判断ログが記録されているか（セッション中にリアルタイムで記録済みの場合がある）
...
1. セッションログのスキャフォールドが既に存在するか確認する:
   ```bash
   ls prompts/runs/$(date +%Y-%m)/
   ```
2. Step 2 の判定で不要（⚪）の場合、スキャフォールドがあれば削除して終了
3. スキャフォールドが存在しない場合は作成する。初回コミット時刻を取得してファイル名を組み立てる:
   ```bash
   git log --format="%ci" --reverse HEAD --not main | head -1
   ```
4. ファイル名を命名規則と照合する（`docs.md` の照合プロセス）
5. スキャフォールドのプレースホルダーを埋め、残りのセクションを仕上げる: 概要 → 背景と目的 → 実施内容 → 設計上の判断 → （判断ログは記録済み）→ 成果物 → 議論の経緯 → 学んだこと → 発見した問題 → 次のステップ
6. サニタイズ規則に従って校正
