...
1. セッションログファイルが既に存在するか確認する（判断ログがリアルタイムに記録されている場合、ファイルが既にある）:
   ```bash
   ls prompts/runs/$(date +%Y-%m)/
   ```
2. ファイルが存在しない場合は作成する。初回コミット時刻を取得してファイル名を組み立てる:
   ```bash
   git log --format="%ci" --reverse HEAD --not main | head -1
   ```
3. ファイル名を命名規則と照合する（`docs.md` の照合プロセス）
4. 残りのセクションを仕上げる: 概要 → 背景と目的 → 実施内容 → 設計上の判断 → （判断ログは記録済み）→ 成果物 → 議論の経緯 → 学んだこと → 発見した問題 → 次のステップ
5. サニタイズ規則に従って校正
