...
レポート保存後、推奨アクションのうち高インパクトのものを Issue 化する。Issue 作成は提案のみ行い、ユーザー確認後に実行する。

手順:

1. 各推奨アクションについて、既存 Issue との重複を確認する:
   ```bash
...
2. 推奨アクションの一覧（重複確認結果付き）をユーザーに提示し、Issue 化の確認を取る
3. 確認後、重複なしのアクションに対して Issue を作成する:
   ```bash
