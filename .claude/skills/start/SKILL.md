...
<!-- 改善: process/improvements/2026-02/2026-02-27_1603_スキル内ステップ遷移での過剰な確認.md -->
## 自律実行の原則

ユーザーが `/start` を起動した時点で、このスキルの全ステップは承認済み。Issue 駆動開発フローの確立されたプロセス（精査 → ブランチ → Draft PR → 設計 → TDD）に従う遷移では、確認を挟まずに実行する。

確認なしで進む遷移:
- Step 1-6 の各ステップ間
- Issue 精査 → ブランチ作成 → Draft PR 作成
- コンテキスト提示 → 設計フェーズ → TDD 実装

確認が必要な場面（破壊的操作・選択を伴う場合のみ）:
- Issue 精査の結果が「再構成」「破棄」の場合
- ユーザーに Issue を選択してもらう必要がある場合
- 設計上の判断で複数選択肢がある場合
...
収集した情報を状態別の形式で提示する。このステップは中間ステップであり、Step 7 に続く。
...
コンテキスト提示（Step 6）の直後に、状態に応じた次のアクションを確認なしで実行する。このステップがスキルの最終ステップであり、ここで作業が開始される。

| ワークフロー状態 | 次のアクション |
|----------------|--------------|
| A: 新規着手 | [Issue 駆動開発 > 設計](../../../.claude/rules/dev-flow-issue.md#4-設計) に従い、設計フェーズを開始する |
| B: PR 欠落（復旧後） | Issue の進捗（完了基準のチェック状態、コミット履歴）から中断箇所を特定し、設計または実装を再開する |
| C: 通常再開 | Issue の実装計画とコミット履歴から中断箇所を特定し、該当 Phase の設計または実装を再開する |
| D: レビュー中 | `/review-and-merge` の実行をユーザーに案内して終了する |

状態 A/B/C では、確認・宣言（「設計に進みます」「実装を再開します」等）で停止せず、直ちにアクションを実行する。→ [自律実行の原則](#自律実行の原則)

**禁止:** Step 6 のコンテキスト提示で停止し、Step 7 を実行しないこと
