...
→ <Issue の実装計画とコミット履歴から特定した、次の具体的アクション>を実行します。
```
...
コンテキスト提示後、確認を挟まずに次のアクション（設計、TDD 等）を実行する。→ [自律実行の原則](#自律実行の原則)

**禁止:** 確立されたフレームワーク（Issue 駆動開発 → 精査 → 設計 → TDD）に従う場合に「進めてよろしいですか？」「設計に進みます」等の確認・宣言で停止すること。確立されたプロセスへの確認は安全策ではなく摩擦である。
