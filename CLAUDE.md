...
<!-- 改善: process/improvements/2026-03/2026-03-08_1914_Issue作成時にオーナーのWantを深掘りせず表面的な完了基準を生成した.md -->
## Want の深掘り

ユーザーの発言に基づいて何かを定義・決定する前に、発言の背後にある本質的なニーズ（Want）を検証する。ユーザーの発言は Want の手がかりであり、そのまま完了基準や方針に転写してよいものではない。

→ Want と How の区別: [`.claude/rules/problem-solving.md`](.claude/rules/problem-solving.md#want-と-how)
...
検証の結果、Want が不明確と判断した場合は自分の理解を提示してユーザーに確認する。Want が明確な場合はそのまま進行する。

### 成果物要件: Issue の Want セクション

→ 経緯: [改善記録: Issue 作成時にオーナーの Want を深掘りせず表面的な完了基準を生成した](process/improvements/2026-03/2026-03-08_1914_Issue作成時にオーナーのWantを深掘りせず表面的な完了基準を生成した.md)
...
**禁止:** ユーザーの発言を深掘りせず、表面的な理解から完了基準や方針に How を混入させること
