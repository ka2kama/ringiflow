...
技術選定や設計判断を行う際は、以下の原則に従う。

### 1. 理念・要件への合致を最優先

目先の効率やシンプルさよりも、プロジェクトの理念・目的・要件への合致が最重要である。
...
悪い例:
> 「UUID v7 は B-tree 効率が良いので全面採用しましょう」

良い例:
> 「要件定義書のセキュリティ要件、ADR-001 の決定理由、ID 設計規約を確認した上で、UUID v4/v7 の使い分けが適切と判断します」
...
既存の ADR を補足する場合（新しい選択肢の検討追加、却下理由の詳細化など）は、ADR の決定内容を変更せず、選択肢と変更履歴のセクションに追記する。
...
| 実装の詳細解説 | [`docs/07_実装解説/`](docs/07_実装解説/) | [`README.md`](docs/07_実装解説/README.md) |
...
**実装解説の役割:**
- AI エージェントが実装した内容の詳細解説
- 設計判断の理由、代替案、トレードオフの説明
- オーナーのレビュー・学習を支援
- Phase 単位で作成
...
**実装解説** — 以下のいずれかに該当する場合:
- Issue の Phase（実装フェーズ）が完了した
- 設計判断を伴う実装を行った

フォーマットは [`docs/07_実装解説/README.md`](docs/07_実装解説/README.md) に従うこと。
...
4. 会話スレッド解決（返信 → resolve）
   - 全ての会話スレッドが resolve されていないとマージできない
   - 必ず対応理由を返信してから resolve する
   - 返信例: 「○○ で修正しました（コミット: abc1234）」
   - 理由なしで resolve しない（対応の追跡可能性を確保するため）

5. マージ前提条件の確認
   - CI が全て通過している
   - approve されている
   - 全ての会話スレッドが resolve 済み
...
- **Issue の進捗を都度更新すること**（チェックボックス、ステータス等）
  - Phase やタスクが完了したら、Issue のチェックボックスを `[x]` に更新
  - `gh issue edit` コマンドで更新可能
