...
失敗タイプは固定（理由: 理論的網羅性）。ヒューマンエラー研究（Reason, 1990）の knowledge-based / rule-based / skill-based error に対応し、行動の失敗原因を網羅的に分類する。(1) 知識がない、(2) 知識はあるが実行しない、(3) プロセス自体が不十分、の 3 分類は相互排他的かつ網羅的であり、新しいタイプの追加は不要。
