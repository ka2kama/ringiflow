...
Issue #1095 / PR #1097 で全て実施済み:

- [x] バックエンド: `ApiResponse::new(data)` → `data` に置換、`ApiResponse` 型を削除
- [x] フロントエンド: `Api.elm` の `expectJson` から `Decode.field "data"` を除去、`getRaw` / `expectJsonRaw` を廃止
- [x] API テスト: Hurl の `jsonpath "$.data.xxx"` → `jsonpath "$.xxx"` に置換
- [x] OpenAPI: アノテーションから `ApiResponse<T>` を除去
- [x] `PaginatedResponse` の `data` フィールドを `items` にリネーム

### 関連ドキュメント

- 実装: `backend/crates/shared/src/paginated_response.rs`
- 実装: `frontend/src/Api.elm`
- 関連 PR: #1092（auto-unwrap 導入）、#1097（エンベロープ廃止の実装）
...
|------|---------|
| 2026-03-09 | 初版作成 |
| 2026-03-09 | アクション項目を実施済みに更新（PR #1097）、削除済みファイルへの参照を除去 |
