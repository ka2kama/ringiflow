...
RingiFlow は本番環境として AWS の ECS Fargate + Aurora PostgreSQL + ElastiCache Redis を想定している（[基本設計書](../30_基本設計書/)参照）。しかし、この構成は月額数百ドル以上のコストがかかる。
...
単一の Lightsail インスタンス（$5/月、1GB RAM）に全コンポーネントをデプロイ。Cloudflare Free プランで SSL 終端と CDN を提供。
...
1. **コスト最適**: 月額 $5 で全要件を満たせる（初回90日無料）
...
- 1GB RAM の制限があり、本格的な負荷テストは不可（不足時は $12 プラン（2GB）にスケールアップ可能）
...
|------|---------|
| 2026-01-19 | 初版作成 |
| 2026-02-06 | OS を Ubuntu 24.04 LTS から AlmaLinux 9.4 へ変更（RHEL 系の学習効果を重視）。プランを $10（2GB）から $5（1GB）に変更（Rust の軽量さを活かす） |
| 2026-02-14 | IPv6-only からデュアルスタックに切り替え（[ADR-047](047_LightsailのIPv6-onlyからデュアルスタックへの切り替え.md)）。月額 $5 → $7 |
