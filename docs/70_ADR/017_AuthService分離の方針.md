...
RingiFlow の認証機能は現在、以下のように BFF と Core Service に分散している：

| コンポーネント | 責務 |
|---------------|------|
| BFF | セッション管理、Cookie、CSRF 防御、IdP との OIDC 連携 |
| Core Service | パスワード検証（`/internal/auth/verify`）、ユーザー情報管理 |

### 課題

1. **責務の分散**
   - 認証に関する責務が BFF と Core Service に分散しており、将来の拡張時に責務境界が曖昧になるリスクがある
...
2. **将来の認証要件の複雑さ**
   ...
   - これらを Core Service に追加すると、Core Service の責務が肥大化する
...
### 選択肢 1: 現状維持（Core Service に認証ロジックを含める）

Core Service 内に認証ロジックを保持し続ける。
...
- 利点:
  ...
  - サービス間通信のオーバーヘッドなし
- 欠点:
  - Core Service の責務が肥大化するリスク
  - SSO/MFA 追加時に Core Service が複雑化
  - 責務境界が曖昧

### 選択肢 2: Auth Service として分離

認証専用のマイクロサービスを作成し、Core Service から認証ロジックを分離する。
...
- **選択肢 1（現状維持）**: 将来の SSO/MFA 対応時に Core Service が複雑化するリスクが高い
...
現在 Core Service にある認証ロジックを Auth Service として分離する。
...
2. 既存の認証ロジック（Core Service）の移行計画
3. 基本設計書の更新（アーキテクチャ図の修正）
4. Phase 2 の Issue 作成

### 関連ドキュメント

- 要件定義書: [01_コア要件.md - 4.1 認証・認可機能](../10_要件定義書/01_コア要件.md)
- 基本設計書: [01_アーキテクチャ設計.md](../30_基本設計書/01_アーキテクチャ設計.md)
- ナレッジベース: [BFFパターン.md](../80_ナレッジベース/architecture/BFFパターン.md)
