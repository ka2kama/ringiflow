...
| CI 統合 | `pnpm exec jscpd`（ルート devDependencies） | `cargo install` 要 | JDK セットアップ要 |
| メンテナンス | 活発 | 不明 | 活発 |
...
2. Node.js は既存のエコシステム（`pnpm exec redocly` パターンが `lint-openapi` で確認済み）。新しい依存を追加する必要がない
3. ルート `package.json` の devDependencies で管理し、`pnpm exec` で実行。開発者のローカル環境にグローバルインストールが不要（`check-tools` への追加も不要）
