...
flowchart LR
    subgraph ringiflow["ringiflow 独自フレームワーク"]
        ...
        TS["troubleshooting.md<br/>仮説カテゴリチェックリスト"]
        PR["PR レビュー<br/>claude-auto-review.yaml"]
    end
    ...
    subgraph known["既知手法"]
        ...
        FiveM["5M（製造業）"]
        IKW["Ishikawa Diagram"]
        CWC["correlate with changes（SRE）"]
        AVH["可用性ヒューリスティック"]
        GCR["Google Code Review Guidelines"]
        CC["Conventional Comments"]
        BB["Bacchelli & Bird (2013)"]
        SB["SmartBear Cisco 研究"]
    end
...
### SRP — 単一責務原則（Robert C. Martin）

SOLID 原則の S。モジュールの変更理由は 1 つだけであるべきという原則。Robert C. Martin が 2003 年に体系化した。

ringiflow の対応: ギャップ発見の観点「責務の蓄積」が SRP の蓄積的な違反パターンを検出する。既存モジュールへの機能追加で、既存の責務と異なる関心事が導入されるケースを対象とする。

違い: SRP は設計原則として随時適用するが、ringiflow では「ギャップ発見の観点」としてブラッシュアップループの中で体系的に適用する。TDD Refactor の設計原則レンズ（dev-flow-tdd.md）とは異なり、ファイル全体の俯瞰レベルで責務の蓄積を検出する。
...
| 責務の蓄積検出 | SRP（単一責務原則） | 「ギャップ発見の観点」としてブラッシュアップループに組み込み、ファイル全体レベルで蓄積を検出 |
| 文脈依存のダミー値検出 | Making Illegal States Unrepresentable（Minsky） | 「ギャップ発見の観点」として、同一型の異なる使用文脈によるダミー値パターンを設計段階で検出。設計原則レンズの「型の活用」がコードレベルで同原則を適用するのに対し、こちらはブラッシュアップループで型設計の欠陥を検出する |
...
→ 詳細: [TDD 開発フロー > 二層の Red モデル](../../60_手順書/04_開発フロー/02_TDD開発フロー.md#二層の-red-モデル)
...
→ 詳細: [TDD 開発フロー > 操作パスの列挙](../../60_手順書/04_開発フロー/02_TDD開発フロー.md#操作パスの列挙テストリスト作成の前に)
...
- Newtype パターン（[ADR-016](../../70_ADR/016_プリミティブ型のNewtype化方針.md)）: プリミティブ型の取り違えを型で防止
- 型安全ステートマシン（[ADR-054](../../70_ADR/054_型安全ステートマシンパターンの標準化.md)）: 状態依存フィールドを ADT で分離
- 構造的強制（[ADR-051](../../70_ADR/051_トランザクションコンテキストによる構造的強制.md)）: 引数の型でルール遵守を強制
...
→ 詳細: [TDD 開発フロー > 設計原則レンズ](../../60_手順書/04_開発フロー/02_TDD開発フロー.md#設計原則レンズ)
...
ringiflow の対応: UI/UX レンズの「視覚的階層」が対応する。タイポグラフィ階層（[デザインガイドライン](../../40_詳細設計書/13_デザインガイドライン.md)）と近接によるグルーピングを問う。
...
→ 詳細: [TDD 開発フロー > UI/UX レンズ](../../60_手順書/04_開発フロー/02_TDD開発フロー.md#uiux-レンズフロントエンド変更時)

## troubleshooting.md と既知手法

### ステップ 0（立ち止まる）の背景

エラー発生直後に原因調査をせず即座に対処を実行するパターンへの対策。調査フロー（ステップ 1〜8）が存在しても、そもそも調査モードに入らずに即座に対処に飛ぶケースに対応する。

| 本記録の要素 | 対応する既知手法 |
|------------|-----------------|
| ステップ 0（立ち止まる） | 医療分野の Time-Out 手順 — 手術前に全員で立ち止まり患者・手術部位・手技を確認する |
| 3 問の強制チェックリスト | Safety 科学の Stop → Think → Act — 行動前に状況認識を強制するフレームワーク |
| 推測による即座の対処の抑制 | Kahneman の System 1/System 2 — System 1（直感的・高速）の反応を抑え、System 2（分析的・低速）を起動する |
...
改善の経緯: [エラー発生時に原因確認せず即座に対処を実行](../../../process/improvements/2026-03/2026-03-04_2227_エラー発生時に原因確認せず即座に対処を実行.md)

### 仮説カテゴリチェックリストの背景

トラブルシューティングの仮説生成段階でコード起点の調査にバイアスがかかりやすい。仮説カテゴリチェックリスト（6 カテゴリ: コード変更、データ変異、内部設定、インフラ変更、外部サービス、時間依存要因）は、このバイアスに構造的に対抗する仕組み。

### 認知バイアスへの対抗

仮説カテゴリチェックリストが対抗するバイアス:

| バイアス | 内容 | チェックリストの効果 |
|---------|------|-------------------|
| 可用性ヒューリスティック | 手元で確認できるもの（コード、ログ、DB）を優先しがち | 全 6 カテゴリの明示的な検討を強制 |
| 内部帰属バイアス | 原因を自システム内部に求めがち | カテゴリ 5-6（外部サービス、時間依存要因）の明示的な検討 |
| ツール法則 | Read/Grep/DB クエリが得意だと、すべてをそれで解決しようとする | AI が直接確認できないカテゴリの存在を認識させる |

参考: Tversky, A. & Kahneman, D. (1973) "Availability: A Heuristic for Judging Frequency and Probability"

### 5M（Man, Machine, Material, Method, Environment）

製造業で確立された原因分析のカテゴリ化手法。問題の原因を 5 つのカテゴリに分類して探索する。

ringiflow の対応: 仮説カテゴリチェックリストの 6 カテゴリが 5M の構造を参照している。ソフトウェアの文脈に合わせて「Man」を除外し、代わりに「データ変異」「時間依存要因」を追加している。
...
| Method | コード変更、内部設定 |
| Environment | 外部サービス |
| Man | （除外: AI エージェントの文脈では不要） |
| — | 時間依存要因（5M にない追加カテゴリ） |

### Ishikawa Diagram（特性要因図）

石川馨が 1960 年代に考案した原因分析手法。魚骨図（fishbone diagram）とも呼ばれ、問題の原因を大骨（カテゴリ）と小骨（具体的原因）に構造化する。

ringiflow の対応: 仮説カテゴリチェックリストの「カテゴリ → 探索対象」の構造が、Ishikawa Diagram の大骨 → 小骨に対応する。特に「環境」カテゴリ（外部要因）を明示的に探索空間に含める点が共通する。

参考文献: Ishikawa, K. (1985) "What is Total Quality Control? The Japanese Way"

### SRE の "correlate with changes"

Google の SRE（Site Reliability Engineering）で確立されたインシデント調査手法。「最近何が変わったか？」を最初に問うことで、変更と障害の相関を迅速に発見する。

ringiflow の対応: 仮説カテゴリチェックリストのカテゴリ 1-4（コード変更、データ変異、内部設定、インフラ変更）がこの着眼点に対応する。内→外の順に検討することで、変更の影響を系統的に追跡する。

参考文献: Beyer, B. et al. (2016) "Site Reliability Engineering: How Google Runs Production Systems", Chapter 12

### ringiflow の独自性

| 要素 | 既知手法に存在するか | ringiflow 独自の追加 |
|------|---------------------|---------------------|
| 原因カテゴリの構造化 | 5M、Ishikawa Diagram | ソフトウェア文脈への適応（データ変異、時間依存要因の追加） |
| 変更との相関調査 | SRE "correlate with changes" | カテゴリ 1-4 として構造化し、内→外の検討順序を定義 |
| 認知バイアスの明示 | 行動経済学（Kahneman） | AI エージェント特有のバイアス（ツール法則）の追加 |
| カテゴリ網羅表 | — | 全カテゴリの該当/非該当/未確認を追跡する仕組み |
| 人間への委譲判断 | — | AI が確認できないカテゴリの早期エスカレーション |
| ステップ 0（立ち止まる） | 医療 Time-Out、Safety 科学 Stop → Think → Act | 調査フローに入る前のゲートとして 3 問の強制チェックリストを追加 |

→ 詳細: [troubleshooting.md](../../../.claude/rules/troubleshooting.md)
...
## claude-auto-review.yaml（PR レビュー）と既知手法

### 概要

ringiflow の自動 PR レビューシステム（`claude-auto-review.yaml` + `claude-rules-check.yaml`）は、以下の既知手法を参照して設計されている。

| ringiflow の仕組み | 既知手法 |
|-------------------|---------|
| V&V のジョブ分離（Verification / Validation） | V&V（Boehm / IEEE 1012） |
| Verification の 7 観点 | Google Code Review Guidelines |
| 重大度ラベルと blocking/non-blocking の区別 | Conventional Comments、Google Nit: 概念 |
| Rules Check の重大度階層 | Conventional Comments の blocking/non-blocking デコレータ |
| コメントに「なぜ問題か」を含める | Google "What to Look For"、Bacchelli & Bird (2013) |
| 確信度の明示 | AI コードレビュー研究（偽陽性問題） |
| コメント数のガイドライン | SmartBear Cisco 研究（alert fatigue） |

### Google Code Review Guidelines

Google が公開している Engineering Practices のコードレビューガイドライン。レビューの目的、見るべき観点、コメントの書き方、重大度の表現を体系化している。

ringiflow の対応: Verification プロンプトのレビュー観点が Google の観点リストを参照し、AI レビューに適した形で再構成している。

| Google の観点 | ringiflow の対応 |
|--------------|-----------------|
| Design（設計） | 設計・複雑さ |
| Functionality（機能性） | 正確性・ロジック |
| Complexity（複雑さ） | 設計・複雑さ（統合） |
| Tests（テスト） | テストの妥当性 |
| Naming（命名） | コード改善（non-blocking） |
| Comments（コメント） | コード改善（non-blocking） |
| Style（スタイル） | Rules Check の責務（Verification の対象外） |
| Documentation（ドキュメント） | Validation の責務 |

Google 固有の「改善 > 完璧」の原則は、ringiflow の「問題がなければ躊躇なく承認する」ポリシーに反映されている。

重大度ラベル:

| Google の概念 | ringiflow の対応 |
|--------------|-----------------|
| Nit: | Medium/Low（non-blocking） |
| Optional / Consider | コード改善カテゴリ（non-blocking） |
| FYI | 学習機会の提供（承認判断に影響しない） |

参考: [Google Engineering Practices - Code Review](https://google.github.io/eng-practices/review/)

### Conventional Comments

コードレビューコメントの構造化フォーマット。ラベル + デコレータ + 本文で、コメントの意図と緊急度を明確にする。

ringiflow の対応: Conventional Comments のフルフォーマットは採用せず、blocking/non-blocking の概念を重大度モデルに取り込んでいる。

| Conventional Comments | ringiflow の対応 |
|----------------------|-----------------|
| `issue (blocking):` | Critical/High（request-changes） |
| `suggestion (non-blocking):` | Medium/Low（approve + コメント） |
| `nitpick:` | Low（non-blocking） |
| `praise:` | 不採用（AI の賞賛は偽陽性のリスクが高い） |
| `question:` | 不採用（AI の非同期質問は非効率） |

フルフォーマットを不採用とした理由: AI レビューのコメントは構造化メタデータ（`<!-- review-metadata -->`）で機械的に分類されるため、人間向けのテキストラベルは冗長になる。重大度モデルで概念を取り込み、フォーマット自体は簡潔に保つ。

参考: [Conventional Comments](https://conventionalcomments.org/)

### Bacchelli & Bird (2013) — Modern Code Review 研究

Microsoft での 165 人のマネージャーと 873 人のプログラマーへの調査。コードレビューの期待と実際の成果のギャップを明らかにした。

主要な知見:
- 期待される成果の 1 位は欠陥発見だが、実際の成果の 1 位はコード改善（スタイル、可読性）
- 知識移転とチーム意識の向上もレビューの重要な価値

ringiflow の対応:
- AI レビューは「コード改善」と「定型的な欠陥発見」を担当
- 「知識移転」は学習機会の提供として一方向的に実現（AI → 人間）
- 「チーム意識」「アーキテクチャ判断」は AI では代替できないため、人間レビューの価値として明示

参考文献: Bacchelli, A. & Bird, C. (2013) "Expectations, Outcomes, and Challenges of Modern Code Review", ICSE 2013

### SmartBear Cisco 研究（レビュー効率）

Cisco Systems でのコードレビューのデータ分析。レビューサイズ、速度、時間と欠陥発見率の関係を定量化した。

主要な知見:
- 最適なレビューサイズは 200-400 LOC（超えると発見率が低下）
- 60 分を超えるとレビュアー疲労でパフォーマンスが低下
- 大量のコメントは読まれない（alert fatigue）

ringiflow の対応:
- AI レビューに「疲労」はないが、大量のコメントによる alert fatigue は人間側で発生する
- コメント数のガイドライン: 重要な問題に絞り、些細な指摘を大量に出さない
- LGTM コメントの簡略化で情報ノイズを削減

参考: SmartBear, "Best Kept Secrets of Peer Code Review" (Cisco Systems study)

### AI コードレビュー研究

2024-2025 年の研究で明らかになった AI コードレビューの特性。

| 領域 | AI の能力 |
|------|---------|
| パターンベースの欠陥検出 | 高精度（ヌルチェック漏れ、アンチパターン） |
| スタイル・一貫性 | 高精度 |
| ロジックエラー | 人間の 1.75 倍見逃す |
| セキュリティ欠陥 | XSS は 2.74 倍見逃す |
| アーキテクチャ判断 | 組織のコンテキスト不足で困難 |

偽陽性（False Positive）問題:
- FPR（False Positive Rate）が 15% を超えると alert fatigue が発生
- 無視されるアラートは最大 40% に達する
- 低確信度の指摘は明示的にラベル付けすることで信頼性を維持

ringiflow の対応:
- 確信度が低い指摘には「確信度: 中/低」を明示
- セキュリティ・パフォーマンスは確信度が高いもののみ指摘
- AI が判断できない領域（設計判断、ドメイン知識）を明示

参考:
- Rethinking Code Review Workflows with LLM Assistance (2025)
- Osmani, A. "Code Review in the Age of AI"

### ringiflow の独自性

| 要素 | 既知手法に存在するか | ringiflow 独自の追加 |
|------|---------------------|---------------------|
| V&V のジョブ分離 | V&V（概念として） | CI ワークフローの物理的なジョブ分離として実装。Verification は Opus、Validation は Sonnet と異なるモデルで実行 |
| Rules Check の責務分離 | Conventional Comments の構造化 | コード品質レビューとルール準拠チェックを完全に別ワークフローに分離。Rules Check は `gh pr review` を使用せず承認判断に影響しない |
| 増分レビューモード | — | 前回レビュー後の差分のみを対象とする再レビュー最適化 |
| メタデータ埋め込み | — | レビューコメントに機械可読なメタデータを埋め込み、finalize ジョブで集約 |
| 確信度の明示 | AI 研究の推奨 | Validation に確信度（高/中/低）を必須化。Verification は重大度で代替 |

→ 詳細: [ADR-063](../../70_ADR/063_PRレビュープロンプトのベストプラクティス起点再設計.md)
...
- SmartBear, "Best Kept Secrets of Peer Code Review" (Cisco Systems study)
- Osmani, A. "Code Review in the Age of AI"
...
| 日付 | 変更内容 |
|------|---------|
| 2026-03-05 | zoom-rhythm.md セクションに「文脈依存のダミー値検出」と MISR の対応を追加。Mermaid 図に `ZR --> MISR` 接続を追加（#1049） |
| 2026-03-04 | PR レビュー（claude-auto-review.yaml）セクションを追加。Google Code Review Guidelines、Conventional Comments、Bacchelli & Bird、SmartBear、AI コードレビュー研究を記載（#1036） |
| 2026-03-04 | ステップ 0（立ち止まる）セクションを追加。医療 Time-Out、Safety 科学 Stop → Think → Act、Kahneman System 1/System 2 を記載（#1024） |
| 2026-02-25 | troubleshooting.md セクションを追加。5M、Ishikawa Diagram、SRE correlate with changes、認知バイアスを記載（#919） |
