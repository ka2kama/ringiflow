...
Dependabot がセキュリティアラートを検出した場合の対応方法。

### 対応フロー

1. アラートの詳細を確認（GitHub Security タブまたは `gh api`）
2. 影響を受けるパッケージと依存関係を調査
3. 修正方法を選択して実施
4. ADR に記録（判断を伴う場合）
...
| 方法 | 説明 | 使いどころ |
|------|------|-----------|
| 直接更新 | `package.json` の依存バージョンを上げる | 直接依存の場合 |
| overrides | パッケージマネージャの機能で間接依存を上書き | 上流が未修正の間接依存 |
| 上流待ち | 上流パッケージの修正を待つ | 低リスクで急ぎでない場合 |
| 代替移行 | 別のパッケージに移行 | 根本的な解決が必要な場合 |

### pnpm overrides の使い方

間接依存のバージョンを強制的に上書きする:

```json
{
  "pnpm": {
    "overrides": {
      "vulnerable-pkg@<1.2.3": "1.2.3"
    }
  }
}
```

バージョンセレクタを使うことで、影響範囲を脆弱なバージョンのみに限定できる。

設定後は `pnpm install` で lockfile を更新する。

### 運用上の注意

- overrides は上流が修正されても自動反映されない
- 定期的に overrides が不要になったか確認する
- ADR に記録しておくと、将来の判断材料になる

### 関連 ADR

- [ADR-018: cross-spawn 脆弱性への対応](../../70_ADR/018_cross-spawn脆弱性への対応.md)
...
| 2026-01-19 | セキュリティアラート対応セクションを追加 |
