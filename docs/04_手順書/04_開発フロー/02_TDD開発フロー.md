...
4. **結果を記録する**: 計画ファイルのチェックボックスを `[x]` に更新し、確認結果を1行で追記する
...
# 実施後
- [x] 型: UserState の定義 → `handler/auth.rs` L23, 3バリアント (Active/Inactive/Pending)
```
...
ハンドラテスト:
- [ ] Core Service: 正しい認証情報で 200 を返す
- [ ] Core Service: 不正な認証情報で 401 を返す
- [ ] BFF: セッション Cookie を設定する
- [ ] BFF: テナント分離が機能する

API テスト:
- [ ] POST /api/v1/auth/login で認証成功
- [ ] POST /api/v1/auth/login で認証失敗

E2E テスト:
- [ ] ログイン画面からログイン操作を完了できる
```
