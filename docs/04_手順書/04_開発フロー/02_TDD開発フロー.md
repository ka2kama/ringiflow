...
| 型の活用 | 型で防げる不正状態を String や整数で済ませていないか？（[ADR-016](../../../docs/05_ADR/016_プリミティブ型のNewtype化方針.md)） 状態によって有効なフィールドが異なるのにフラットな型にしていないか？（[ADR-054](../../../docs/05_ADR/054_ADTベース状態マシンパターンの標準化.md)） |
