...
実装計画を作成したら、自己検証プロトコルを実施する。これはこのプロジェクトの核となるプラクティスであり、省略してはならない。
...
2. **設計・計画チェックリストで検証する**
   ...
   - 詳細: [自己検証プロトコル](../../../.claude/rules/self-review.md)

3. **検証結果を計画に記載する**
   - PR 本文または Issue に自己検証セクションを追加
   - 各項目の判定と確認内容を明記
...
**自己検証（4.4）は省略不可。** 簡略化する場合でも、最低限の To-Be 定義とスコープ確認は行う。
...
#### 6.3 自己検証結果を PR 本文に記載

[自己検証プロトコル](../../../../.claude/rules/self-review.md) の実装フェーズ・横断検証フェーズを実施し、結果を **PR 本文の Self-review セクション** に記載する。
...
- 自己検証結果が記載されていない PR は Ready for Review にしない
