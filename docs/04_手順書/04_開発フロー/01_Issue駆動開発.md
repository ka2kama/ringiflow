...
注: 操作パスの分類 — 正常系: 期待通りの成功、準正常系: 設計されたエラー処理（バリデーションエラー等）、異常系: 想定外の障害。日本の IT 現場での慣用分類。ISTQB では positive testing / negative testing の 2 分類。
...
改善の経緯: [子 Issue 分割実装時の操作パス検証欠落](../../../process/improvements/2026-02/2026-02-21_1700_子Issue分割実装時のUI操作テスト欠落.md)
