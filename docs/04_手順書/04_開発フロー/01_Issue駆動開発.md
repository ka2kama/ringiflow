...
品質チェックリスト（6.2）の確認結果を **PR 本文の Self-review セクション** にカテゴリ別チェックボックスで記載する。

各カテゴリに対して:
- チェック（`[x]`）+ 確認内容を 1 行で記載する
- 該当しないカテゴリは `[x]` + `N/A（理由）` で明示する
- 全行が `[x]` であること（未確認の `[ ]` を残さない）
...
- [x] 設計・ドキュメント: 詳細設計書更新済み、ADR該当なし、セッションログ作成済み
- [x] Issue との整合: 完了基準3件・実装計画4Phase すべて [x] 更新済み
- [x] テスト: テストリスト全12項目実装、ユニット/ハンドラ/API/E2E全層カバー
...
注意:
- チェックだけでなく、具体的に何を確認したかを記載する（rubber-stamp 防止）
