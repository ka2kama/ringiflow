...
自動チェック通過後、以下を確認する。整合性（マイナス→ゼロ）と品質向上（ゼロ→プラス）の両面を確認する。
...
- [ ] 同じ結果をより少ない概念で実現できないか（不要な中間層、冗長なデータ構造）
- [ ] 型で防げる不正状態を String や整数で済ませていないか
- [ ] 各コンポーネントの責務が単一か（複数の関心事が混ざっていないか）
- [ ] その分野の推奨パターンに沿っているか（標準ライブラリ、フレームワークの活用）
...
| 日付 | 変更内容 |
|------|---------|
| 2026-02-06 | 6.2 に品質向上（ゼロ→プラス）チェックを追加、セクション名を「品質チェックリスト」に変更 |
