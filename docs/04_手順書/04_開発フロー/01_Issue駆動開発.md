...
| テスト計画 | ✓ | 完了したテスト項目を `[x]` で記載 |

オプションセクション（必要な場合のみ追加）:
- **Changes** — 概要で足りない場合の詳細な変更点
- **Notes** — 設計判断の背景、レビュアーへの補足

注意:
- 空のセクションは削除する
- HTML コメント（`<!-- -->`）は使用しない
- テスト計画には完了項目のみ記載（未完了 `[ ]` を残さない）
...
- 品質ゲート: 品質チェックリスト + セルフレビュー + 収束確認。品質を担保するプロセス
...
品質チェックリスト（6.2）の確認結果を **PR 本文のセルフレビューセクション** にカテゴリ別の確認内容として記載する。
...
- 「設計・ドキュメント」カテゴリでは、作成・更新したドキュメントの具体的なファイルパスを記載する。「作成済み」「更新済み」のみの記載は検証不能なため不可。該当なしの場合は `ADR該当なし` のように明示する
...
- コミットメッセージ: PR タイトル + PR 本文（概要, テスト計画等）
...
| 日付 | 変更内容 |
|------|---------|
| 2026-02-24 | PR テンプレートのセクション名を日本語化（Self-review→セルフレビュー、Test plan→テスト計画、Summary→概要）、関連する参照をすべて更新（#897） |
| 2026-02-24 | 品質チェックリスト 6.2 にセキュリティ基本確認項目を追加、ISO 25010 品質特性とのマッピング参照リンクを追加（#894） |
| 2026-02-24 | 品質チェックリスト 6.2 の型安全性（マイナス→ゼロ）と型の活用（ゼロ→プラス）の役割分担を明確化。既存パターン準拠確認 vs 新規改善機会に再定義（#891） |
...
| 2026-02-21 | 6.3 セルフレビューをチェックボックスから検証レポート形式（根拠付きリスト）に変更（#749） |
| 2026-02-21 | 6.3 セルフレビューをカテゴリ別チェックボックス形式に構造化（#745） |
