...
→ 機能仕様書: [06_ドキュメント管理.md](../20_機能仕様書/06_ドキュメント管理.md)
...
CREATE TABLE folders (
    ...
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    -- 同一フォルダ内で名前の重複を防ぐ（NULLS NOT DISTINCT でルートレベルも保証）
...
- [コア要件（ドキュメント管理機能）](../10_要件定義書/01_コア要件.md) 4.4 節
- [インフラと DB 設計（S3 バケット）](../30_基本設計書/03_インフラとDB設計.md)
