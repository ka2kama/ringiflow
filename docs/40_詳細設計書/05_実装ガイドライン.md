...
→ [Elm Ports（JavaScript 連携）](../80_ナレッジベース/elm/Elmポート.md)
...
> **実装状態**: 部分的に実装済み。基本的なログ出力は実装済み。`correlationId` の全レイヤー伝播は Phase 4 で対応予定。
...
→ [Rust エラーハンドリング](../80_ナレッジベース/rust/Rustエラーハンドリング.md)
...
> **実装状態**: 部分的に実装済み。楽観的ロック（`version` カラム）は実装済み。`commandId`/`eventId` による冪等性保証は Phase 4（イベント駆動）で対応予定。
...
- アーキテクチャ概要: [30_基本設計書/00_アーキテクチャ概要.md](../30_基本設計書/00_アーキテクチャ概要.md)
