...
→ 機能仕様書: [04_ワークフローデザイナー.md](../20_機能仕様書/04_ワークフローデザイナー.md)
→ 技術選定: [ADR-053](../70_ADR/053_ワークフローデザイナー技術選定.md)
    ...
    "fields": [
      {
        ...
        "min": "number（number のみ、任意）",
        "max": "number（number のみ、任意）",
...
| `form.fields[].min` | number | - | 最小値（number のみ） |
| `form.fields[].max` | number | - | 最大値（number のみ） |
...
- `Data/WorkflowDefinition.elm` — 定義データモデル・デコーダー（既存を拡張）
- `Api/WorkflowDefinition.elm` — CRUD API 呼び出し（既存を拡張）
...
type DraggingState
    ...
    | DraggingReconnection Int ReconnectEnd Position  -- transition index, 端点, マウス位置
...
type Msg
    ...
    | TransitionEndpointMouseDown Int ReconnectEnd Float Float  -- 接続線端点のドラッグ開始
...
- [コア要件 CORE-11](../10_要件定義書/01_コア要件.md)
...
| 日付 | 変更内容 |
|------|---------|
| 2026-02-24 | DraggingReconnection バリアントと TransitionEndpointMouseDown Msg を追加（#907） |
| 2026-02-20 | 初版作成 |
