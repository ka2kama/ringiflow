...
- [ADR-017: Auth Service 分離の方針](../70_ADR/017_AuthService分離の方針.md)
- [ADR-022: バックエンドサービスの命名規則と層構造の統一](../70_ADR/022_バックエンドサービスの命名規則と層構造の統一.md)
...
`auth.credentials.user_id` から `public.users.id` への FK 制約は**意図的に設けない**。詳細は [ナレッジベース: マイクロサービス間のデータ整合性](../80_ナレッジベース/architecture/マイクロサービス間のデータ整合性.md) を参照。
