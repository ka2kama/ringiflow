...
RingiFlow の Observability（可観測性）基盤の設計を定義する。運用設計書（`docs/30_基本設計書/04_運用設計.md` セクション 9.1〜9.4）の要件（WHAT）と実装（HOW）を橋渡しし、Phase 4（OpenTelemetry + Datadog）への移行パスを明確にする。

### 関連ドキュメント

- 運用設計書: [セクション 9.1〜9.4](../30_基本設計書/04_運用設計.md#ops-05-監視運用要件)（監視・ログ管理の要件）
- 運用設計書: [セクション 8.5〜8.6](../30_基本設計書/04_運用設計.md#85-監査ログ要件)（監査ログ、鍵管理要件）
- ADR-045: [DynamoDB による監査ログストア設計](../70_ADR/045_DynamoDBによる監査ログストア設計.md)
- ADR-049: [サービス間共通コード抽出の方針](../70_ADR/049_サービス間共通コード抽出の方針.md)
- ナレッジベース: [ログスキーマ](../80_ナレッジベース/backend/log-schema.md)
...
4. [ログスキーマ](../80_ナレッジベース/backend/log-schema.md)のアクション一覧を更新
...
以下の条件をすべて満たすイベントは、手動のログ出力（`tracing::info!` 等）や `log_business_event!` の対象外とする。tower-http による自動リクエストログは本基準の対象外。
...
詳細な命名規約とコード例は[ログスキーマ > メッセージ書き方ガイドライン](../80_ナレッジベース/backend/log-schema.md#メッセージ書き方ガイドライン)を参照。
...
フィールド一覧は [ログスキーマ](../80_ナレッジベース/backend/log-schema.md) を参照。
...
フィールドスキーマの詳細は [ログスキーマ > Canonical Log Line フィールド](../80_ナレッジベース/backend/log-schema.md#canonical-log-line-フィールド) を参照。
