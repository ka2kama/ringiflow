...
### 対象外（Phase 2 以降）
- 認証情報分離（auth スキーマ） → [08_AuthService設計.md](./08_AuthService設計.md)
- ドキュメント管理
- 監査ログ（DynamoDB）
- イベントストア（DynamoDB）
- 全文検索（OpenSearch）
...
詳細: [ADR-001: ID 形式の選定](../70_ADR/001_ID形式の選定.md)、[04_ID設計規約.md](04_ID設計規約.md)
...
erDiagram
    ...
    users {
        ...
        varchar email
        varchar name
        varchar password_hash "(Phase 2 で削除予定)"
        varchar status
...
Auth Service 分離に伴い追加予定。詳細は [08_AuthService設計.md](./08_AuthService設計.md) を参照。
...
erDiagram
    ...
    auth_credentials {
        uuid id PK
        uuid user_id FK "(外部キー制約なし)"
        uuid tenant_id
        varchar credential_type "password / totp / oidc / saml"
...
| password_hash | VARCHAR(255) | YES | - | パスワードハッシュ（Phase 2 で auth.credentials に移行後、削除予定） |
...
詳細は [07_認証機能設計.md](./07_認証機能設計.md) を参照。
...
```
キー: csrf:{tenant_id}:{session_id}
型: String
TTL: 8時間（28800秒、セッションと同じ）
値: 64文字の暗号論的ランダム文字列（hex エンコード）
```
...
| 2026-01-22 | Auth Service 分離に伴う更新（auth スキーマ追加、Redis キー形式修正） | - |
