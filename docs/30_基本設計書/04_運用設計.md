...
| feature | `feature/<Story番号>-<description>` | 新機能開発（Story 単位） |
| fix | `fix/<Issue番号>-<description>` | バグ修正 |
| hotfix | `hotfix/<Issue番号>-<description>` | 緊急バグ修正 |

Story-per-PR: 各 Story を個別 PR で main にマージする。Integration branch（develop 等）は使用しない。

→ 意思決定: [ADR-046](../70_ADR/046_Story-per-PRブランチ戦略.md)
