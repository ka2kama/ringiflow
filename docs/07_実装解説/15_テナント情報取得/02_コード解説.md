...
| `UserState<R, T>` | [`handler/auth.rs`](../../../backend/apps/core-service/src/handler/auth.rs) | UserRepository + TenantRepository を保持する axum 共有 State |
| `UserWithPermissionsData` | [`handler/auth.rs`](../../../backend/apps/core-service/src/handler/auth.rs) | Core Service → BFF のレスポンス DTO（`tenant_name` 追加） |
| `MeResponseData` | [`handler/auth.rs`](../../../backend/apps/bff/src/handler/auth.rs) | BFF → フロントエンドのレスポンス DTO |
