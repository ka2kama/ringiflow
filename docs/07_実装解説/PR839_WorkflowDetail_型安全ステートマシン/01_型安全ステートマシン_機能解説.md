...
[ADR-054](../../05_ADR/054_型安全ステートマシンパターンの標準化.md) は、状態によって有効なフィールドが異なる場合に ADT で状態を分離するパターンを標準化した。Richard Feldman の "Making Impossible States Impossible"（2016）を Elm コミュニティの確立パターンとして明文化したもの。
