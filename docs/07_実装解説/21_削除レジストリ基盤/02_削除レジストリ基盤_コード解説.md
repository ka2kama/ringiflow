...
| `PostgresUserDeleter` | [`deletion/postgres_user.rs`](../../../backend/crates/infra/src/deletion/postgres_user.rs) | users テーブル削除 |
| `PostgresRoleDeleter` | [`deletion/postgres_role.rs`](../../../backend/crates/infra/src/deletion/postgres_role.rs) | roles テーブル削除 |
...
| `PostgresDisplayIdCounterDeleter` | [`deletion/postgres_display_id.rs`](../../../backend/crates/infra/src/deletion/postgres_display_id.rs) | display_id_counters テーブル削除 |
...
async fn delete(&self, tenant_id: &TenantId) -> Result<DeletionResult, InfraError> {
    ...
    loop {
        let mut query = self.client
            ...
            .expression_attribute_values(":pk", /* tenant_id */)
...
- ① パーティションキー `pk` に `tenant_id` を指定して Query
...
- [詳細設計書: テナント退会時データ削除設計](../../03_詳細設計書/06_テナント退会時データ削除設計.md)
- [ADR-007: テナント退会時のデータ削除方針](../../05_ADR/007_テナント退会時のデータ削除方針.md)
