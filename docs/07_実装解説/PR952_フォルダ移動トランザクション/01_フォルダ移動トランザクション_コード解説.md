...
| `MockFolderRepository` | [`backend/crates/infra/src/mock.rs`](../../../backend/crates/infra/src/mock.rs) | テスト用インメモリ実装 |
...
### 1. MockFolderRepository のインメモリ実装

場所: `backend/crates/infra/src/mock.rs`
...
impl FolderRepository for MockFolderRepository {
    async fn max_subtree_depth(&self, path: &str, _tenant_id: &TenantId) -> Result<i32, InfraError> {
        let folders = self.folders.lock().unwrap();
        let max = folders
            .iter()
            .filter(|f| f.path().starts_with(path))  // パスプレフィックスマッチ
            .map(|f| f.depth())
            .max()
            .unwrap_or(0);
        Ok(max)
...
- [ADR-051: トランザクションコンテキストによる構造的強制](../../05_ADR/051_トランザクションコンテキストによる構造的強制.md)
