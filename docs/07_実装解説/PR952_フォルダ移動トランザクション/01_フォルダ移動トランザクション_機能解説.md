...
プロジェクトでは [ADR-051](../../05_ADR/051_トランザクションコンテキストによる構造的強制.md) に基づき、トランザクションが必要な操作には型レベルでの構造的強制を適用する。書き込みメソッドが `&mut TxContext` を引数に要求することで、非トランザクションでの書き込みがコンパイルエラーになる。
