...
意思決定の詳細は [ADR-007: テナント退会時のデータ削除方針](../04_ADR/007_テナント退会時のデータ削除方針.md) を参照。
...
| users | ◎ email, name, password_hash | CASCADE | tenant_id 経由で削除 |
...
```yaml
# .github/workflows/data-store-check.yml
...
|------|---------|
| 2025-01-15 | 初版作成 |
| 2025-01-15 | 多層防御による漏れ防止機構を追加（型システム、削除レジストリ、テスト、CI、Claude Code ルール） |
| 2025-01-15 | 今後の検討事項セクションを追加 |
