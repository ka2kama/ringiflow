...
| クレート | 役割 | 依存先 |
|---------|------|--------|
| `shared` | 共通ユーティリティ（Correlation ID、日時等） | なし（最も内側） |
| `domain` | エンティティ、値オブジェクト、ドメインエラー | `shared` のみ |
| `infra` | リポジトリ実装、外部サービスクライアント | `domain`, `shared` |
| `api` | HTTP ハンドラ、ルーティング | 全クレート |

**重要: domain は infra に依存しない（依存性逆転の原則）**

- `domain` はリポジトリの**インターフェース**（trait）を定義する
- `infra` はそのインターフェースを**実装**する
- これにより、ドメインロジックがデータベースや外部サービスの詳細に依存しない
...
**Alpine イメージを採用する理由:**
- イメージサイズが小さい（ダウンロード時間短縮）
- セキュリティ更新が速い
- 本番でも Alpine 系を使用予定（一貫性）

**Healthcheck の意図:**
- コンテナ起動完了を検知
- `just setup` で「準備完了まで待機」を実現
- CI で「DB 起動前にテスト実行」を防止

```yaml
healthcheck:
  test: ["CMD-SHELL", "pg_isready -U ringiflow"]
  interval: 5s
  timeout: 5s
  retries: 5
```
...
**名前付きボリュームの意図:**
- コンテナを削除してもデータを保持
- `docker compose down -v` で明示的に削除可能
- 開発中の DB データを維持
