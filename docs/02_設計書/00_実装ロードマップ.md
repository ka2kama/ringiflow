...
手順書（[`docs/03_手順書/`](../03_手順書/)）に従い、以下の順序で実行する:

1. リポジトリ構造構築 → ディレクトリ、justfile、rust-toolchain.toml
2. Rust プロジェクト構築 → Cargo workspace、BFF/Core API スケルトン
...
2. Core API 実装（エンティティ、リポジトリ、ユースケース）
