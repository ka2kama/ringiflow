...
ブラウザで `http://localhost:5173/` を開き、ページが表示されることを確認。

### 2.3 データベース接続確認

```bash
# Docker 経由で接続
docker exec -it ringiflow-postgres psql -U ringiflow -d ringiflow_dev -c "SELECT 1"
```
...
→ 参照: [`/infra/docker/docker-compose.yml`](/infra/docker/docker-compose.yml)
...
→ 参照: [`/apps/api/migrations/`](/apps/api/migrations/)
...
セットアップが完了したら、[`00_Phase0_概要.md`](00_Phase0_概要.md) の完了条件を確認する。
