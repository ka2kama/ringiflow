...
技術選定の詳細は [ADR-002: フロントエンドツールチェーンの選定](../04_ADR/002_フロントエンドツールチェーンの選定.md) を参照。
...
理由: これらの npm パッケージはネイティブバイナリを配布しており、一部のパッケージマネージャー（pnpm 等）ではローカルインストール時に正しく動作しない。詳細は [ADR-002](../04_ADR/002_フロントエンドツールチェーンの選定.md) を参照。
...
データベースマイグレーション管理ツール。
詳細は [sqlx-cli 技術ノート](../05_技術ノート/sqlx-cli.md) を参照。
...
開発環境構築が完了したら、`00_Phase0_概要.md` を参照し、Phase 0 の手順を開始する。
