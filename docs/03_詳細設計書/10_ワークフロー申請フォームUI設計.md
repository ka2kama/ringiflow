...
flowchart TB
    subgraph Main["Main.elm"]
        MainModel["Model<br/>- key: Nav.Key<br/>- route: Route<br/>- session: Session<br/>- page: Page"]
        MainMsg["Msg<br/>- LinkClicked<br/>- UrlChanged<br/>- SessionMsg<br/>- WorkflowNewMsg<br/>- ..."]
    end

    subgraph Session["Session.elm"]
        SessionModel["Model<br/>- user: Maybe User<br/>- tenantId: String<br/>- csrfToken: String"]
    end
...
認証情報・テナント情報など、全ページで必要なグローバル状態を管理する。
...
toRequestConfig session =
    ...
    , csrfToken = Just session.csrfToken
    }
```
...
|-----------|------|------------------|
| **2-1** | Session モジュール作成 | `Session.elm` |
...
```elm
changeRouteTo : Route -> Session -> ( Page, Cmd Msg )
