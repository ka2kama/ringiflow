...
sequenceDiagram
    ...
    participant Core as Core API (port 13001)
...
| レイヤー | 責務 |
|---------|------|
| **BFF** | セッション管理、Cookie 処理、CSRF 防御、Core API への中継 |
| **Core API** | パスワード検証、ユーザー情報取得、ビジネスロジック |
...
詳細: [パスワードハッシュ](../../06_技術ノート/パスワードハッシュ.md)
...
RFC 7807 Problem Details 形式を使用。
...
flowchart LR
    ...
    subgraph Phase4["Phase 4: Core API"]
        D1["AuthUseCase"]
        D2["内部 API ハンドラ"]
    end
...
| 対象 | テストケース |
|------|-------------|
| UserRepository | メールでユーザー検索、存在しないメール、ロール付き取得 |
| PasswordHasher | ハッシュ生成、正しいパスワード検証、不正なパスワード検証 |
| SessionManager | セッション作成/取得/削除、存在しないセッション取得 |
...
| CSRF トークン | POST リクエストにトークンなしで 403 |
