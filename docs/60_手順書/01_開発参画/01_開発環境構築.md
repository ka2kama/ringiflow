...
| ビルドキャッシュ | sccache | 最新 | Rust コンパイル結果のキャッシュ（worktree 間共有） |
| E2E テスト | Playwright | 最新 | ブラウザ E2E テスト（プロジェクトローカル） |
| 依存関係鮮度 | cargo-outdated | 最新 | Cargo 依存関係の更新状況確認 |
| IaC | Terraform | 1.14+ | AWS インフラ管理 |
| リンター | TFLint | 0.61+ | Terraform ベストプラクティス検証 |

技術選定の詳細は [ADR-002: フロントエンドツールチェーンの選定](../../70_ADR/002_フロントエンドツールチェーンの選定.md) を参照。
...
# nightly の rustfmt のみ（unstable features 用）
...
バージョン 22 LTS をインストールする。mise を推奨する（プロジェクトの `.mise.toml` でバージョンが自動管理される）。
...
理由: これらの npm パッケージはネイティブバイナリを配布しており、一部のパッケージマネージャー（pnpm 等）ではローカルインストール時に正しく動作しない。詳細は [ADR-002](../../70_ADR/002_フロントエンドツールチェーンの選定.md) を参照。
...
詳細は [sqlx-cli ナレッジベース](../../80_ナレッジベース/rust/sqlx-cli.md) を参照。
...
選定理由: [ADR-033](../../70_ADR/033_セキュリティスキャニングツールの選定.md)
...
選定理由: [ADR-038](../../70_ADR/038_未使用依存検出ツールの選定.md)
...
Rust のコンパイル結果をキャッシュするツール。2回目以降のビルドを大幅に高速化する。特に git worktree で複数の作業ディレクトリを使う場合、worktree 間でキャッシュを共有できる。

公式: https://github.com/mozilla/sccache

プロジェクトでは `backend/.cargo/config.toml` で `rustc-wrapper = "sccache"` を設定済みのため、インストールするだけで自動的に有効になる。
...
## 20. Playwright のインストール

ブラウザ E2E テストフレームワーク。`just test-e2e` でフロントエンド + バックエンドの統合テストを実行する。

公式: https://playwright.dev/

他のツールと異なり、Playwright はプロジェクトローカル（`tests/e2e/`）にインストールする。`just setup` で依存関係は自動インストールされるが、ブラウザは別途インストールが必要。
...
## 22. rust-script のインストール

単一ファイルの Rust スクリプトを実行するツール。`just check-instrumentation` で使用する。

公式: https://rust-script.org/
...
```bash
rust-script --version
# 出力例: rust-script 0.35.0
```

---

## 23. Terraform のインストール

IaC（Infrastructure as Code）ツール。AWS インフラの定義・管理に使用する。

公式: https://developer.hashicorp.com/terraform

### 23.1 インストール方法

mise でインストールする（プロジェクトの `.mise.toml` でバージョンが自動管理される）。
...
Terraform の静的解析ツール。AWS リソースのベストプラクティス違反を検出する。

公式: https://github.com/terraform-linters/tflint
...
# E2E テスト（プロジェクトローカル）
cd tests/e2e && npx playwright --version && cd ../..
...
### 23.3 IntelliJ IDEA / WebStorm 設定
...
1. **Settings → Languages & Frameworks → Elm**
2. **Lamdera path** に elm のパスを設定（例: `~/.local/share/mise/installs/elm/0.19.1/elm`）
...
| 日付 | 変更内容 | 担当 |
|------|---------|------|
| 2026-03-05 | Terraform / TFLint のインストール手順を追加（IaC 品質検証） | - |
| 2026-02-13 | cargo-outdated のインストール手順を追加（依存関係鮮度チェック） | - |
| 2026-02-11 | Playwright のインストール手順を追加（ブラウザ E2E テスト） | - |
| 2026-02-09 | sccache のインストール手順を追加（ビルドキャッシュ） | - |
