...
Vite は esbuild を開発時のトランスパイルに使用している。

```mermaid
flowchart TB
    subgraph DEV["開発時 (dev server)"]
        direction TB
        A1["ブラウザからリクエスト"] --> A2["Vite が esbuild でトランスパイル（高速）"]
        A2 --> A3["ESM としてブラウザに返却"]
    end

    subgraph PROD["本番ビルド"]
        direction TB
        B1["Rollup でバンドル（最適化重視）"] --> B2["esbuild でミニファイ（高速）"]
    end
```
