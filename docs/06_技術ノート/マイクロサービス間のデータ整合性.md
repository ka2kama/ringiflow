...
sequenceDiagram
    participant Client
    participant CoreAPI as Core Service
    participant AuthService as Auth Service

    Client->>CoreAPI: ユーザー作成リクエスト
    CoreAPI->>CoreAPI: users テーブルに INSERT
    CoreAPI->>AuthService: POST /credentials
    AuthService->>AuthService: credentials テーブルに INSERT
    AuthService-->>CoreAPI: 201 Created
    CoreAPI-->>Client: ユーザー作成完了
```
...
sequenceDiagram
    ...
    CoreAPI->>CoreAPI: users に INSERT
    CoreAPI->>AuthService: POST /credentials

    alt 成功
        AuthService-->>CoreAPI: 201 Created
        CoreAPI-->>CoreAPI: コミット
    else 失敗
        AuthService-->>CoreAPI: エラー
        CoreAPI->>CoreAPI: users から DELETE（補償）
    end
```
...
```mermaid
flowchart LR
    CoreAPI[Core Service] -->|UserCreated イベント| Queue[(メッセージキュー)]
...
[08_AuthService設計.md](../03_詳細設計書/08_AuthService設計.md) で定義:
