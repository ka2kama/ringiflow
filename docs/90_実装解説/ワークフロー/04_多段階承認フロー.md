...
flowchart TD
    ...
    B -->|存在する| C["中間ステップ処理"]
    C --> D["インスタンスを次のステップに進める"]
    C --> E["次のステップを Active にする"]
...
この判定はワークフロー定義から承認ステップの順序リストを取得し、現在のステップの次が存在するかどうかで分岐する。
...
`type == "approval"` のステップが配列順に抽出される。この順序が承認の実行順になる。

### Frontend: ステップ進捗表示

水平プログレスバーでステップの状態を表示する（2ステップ以上の場合のみ表示）。
...
stateDiagram-v2
    state "2段階承認の例" as flow {
        ...
        s1 --> s2: Step1 承認（次ステップに進行）
        s2 --> approved: Step2 承認（全ステップ完了）
...
インスタンスは中間ステップ承認時にステータスが変わらない（InProgress のまま）。変わるのは現在のステップの参照先のみ。これにより、外部から見た「進行中」の意味は一貫する。
