...
| ポートオフセット | スロット番号と一致する決定的マッピング（100 単位） | `scripts/env/generate.sh` |
...
sequenceDiagram
    ...
    Script->>Env: ./scripts/env/generate.sh 1
...
→ [ADR-021](../../70_ADR/021_並行開発環境の構成.md) を参照

### 5. container_name の削除

→ [ADR-021](../../70_ADR/021_並行開発環境の構成.md) の「帰結」を参照
