# Workflow/Detail.elm 型安全ステートマシン - 機能解説
...
### 型安全ステートマシンパターン（ADR-054）

[ADR-054](../../70_ADR/054_型安全ステートマシンパターンの標準化.md) は、状態によって有効なフィールドが異なる場合に ADT で状態を分離するパターンを標準化した。Richard Feldman の "Making Impossible States Impossible"（2016）を Elm コミュニティの確立パターンとして明文化したもの。
...
| 用語 | 説明 | 関連コード |
|------|------|-----------|
| 型安全ステートマシン | 状態ごとに異なる型を定義し、有効なフィールドだけを持たせるパターン | `PageState` |
...
機能・仕組みレベルの判断を記載する。コード実装レベルの判断は[コード解説](./01_型安全ステートマシン_コード解説.md#設計解説)を参照。
