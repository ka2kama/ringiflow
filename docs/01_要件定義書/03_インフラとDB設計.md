...
| 項目 | 要件 | 実装例 |
|------|------|--------|
| アプリケーション層 | 認可ミドルウェアで tenant_id をスレッドローカル/リクエストコンテキストに保持し、Repository 層で自動適用 | axum middleware + Repository wrapper |
| DB 層ガードレール | PostgreSQL の RLS（Row Level Security：行単位でアクセス制御するDB機能）または VIEW による強制フィルタ | SET app.tenant_id + RLS policy |
