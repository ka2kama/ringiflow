...
| DynamoDB | NoSQL データベース。高速な読み書きとスケーラビリティに優れる。監査ログの永続化に使用 |
...
flowchart TB
    ...
    subgraph Backend["Core Service (Rust)"]
        ...
        subgraph DataAccess["Data Access Layer"]
            ...
            DDBRepo["DynamoDB Repo<br/>(Audit Log)"]
            StorageRepo["S3 Repo"]
        end
...
| データアクセス | リポジトリ | SQLx（Rust 用の非同期 SQL ライブラリ） / DynamoDB SDK / S3 SDK |
...
要件定義 [CORE-12](../01_要件定義書/01_コア要件.md#core-12-テスト戦略) で定義されたテストピラミッドに基づき、5 層のテストで品質を担保する。
...
下位のテスト層を厚くし、上位は薄くする。E2E はクリティカルパスのみに集中する。

#### テスト層の定義

| テスト層 | ツール | 目的 | 実行コマンド | 配置場所 |
|---------|--------|------|-------------|---------|
| Unit (Rust) | cargo test + rstest | ドメインロジック・型・状態遷移の検証 | `just test-rust` | `backend/*/src/` |
| Unit (Elm) | elm-test | Decoder・Update ロジックの検証 | `just test-elm` | `frontend/tests/` |
| Integration | sqlx::test | DB 連携・リポジトリの検証 | `just test-rust-integration` | `backend/crates/infra/tests/` |
| API | Hurl | HTTP API 契約・レスポンス構造の検証 | `just test-api` | `tests/api/hurl/` |
| E2E | Playwright | ブラウザを通じたユーザーシナリオの検証 | `just test-e2e` | `tests/e2e/tests/` |

#### 各テスト層の責務境界

| 観点 | Unit / Integration | API（Hurl） | E2E（Playwright） |
|------|-------------------|-------------|-------------------|
| 対象 | 個々の関数・モジュール・DB 連携 | HTTP リクエスト/レスポンス | ブラウザ UI 操作 |
| 検証内容 | ロジックの正確性 | API 契約・ステータスコード・JSON 構造 | ユーザー操作の完結性 |
| 実行速度 | 高速 | 中速 | 低速（ブラウザ起動が必要） |
| 失敗時の原因特定 | 容易 | 中程度 | 困難（複数レイヤーが関与） |

**原則**: 下位テスト層で検証済みの詳細を上位で重複テストしない。E2E は「ユーザーが操作を完了できるか」に集中する。

#### 関連ドキュメント

| ドキュメント | 内容 |
|-------------|------|
| [TDD 開発フロー](../04_手順書/04_開発フロー/02_TDD開発フロー.md) | テスト駆動の開発プロセス |
| [ADR-032: テスト設計方針](../05_ADR/032_テスト設計方針.md) | レイヤーごとの表現形式・命名規則 |
| [API テスト突合表](../08_テスト/APIテスト突合表.md) | OpenAPI 仕様と API テストの対応 |
| [E2E テスト突合表](../08_テスト/E2Eテスト突合表.md) | E2E シナリオとテストの対応 |
| [Hurl ナレッジベース](../06_ナレッジベース/devtools/hurl.md) | API テストツール |
| [Playwright ナレッジベース](../06_ナレッジベース/devtools/Playwright.md) | E2E テストツール |
