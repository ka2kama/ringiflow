...
flowchart TB
    ...
    subgraph Backend["Core API (Rust)"]
...
| Core API | ドメインAPI（内部） | Rust / axum / tower |
...
#### 5.3.2 Rust バックエンド安定運用要件（BFF / Core API / Domain Services）

本節は、Rust（BFF / Core API / Domain Services）側の「壊れにくさ（実行時エラー最小）と安定運用」を担保するための非機能要件を定義する。
本節の要件は、実装方式（axum/tower/sqlx 等）に依存しない運用品質の拘束である。

##### 設計方針（Must）

- **境界の明確化**
  BFF・Core API・Domain Services は、責務境界（認証/認可、入出力検証、ドメイン判断、永続化、非同期処理）を明示し、境界ごとに失敗の扱い（エラー種別、再試行可否、監査要否）を定義する。
...
- **分散トレーシング**
  BFF・Core API・Domain Services・外部IdP・DB/キューの主要区間をトレースし、少なくとも「入口スパン」「依存呼び出しスパン」「イベント処理スパン」を MUST とする。
...
- **保護点の明示**
  BFF は外部（ブラウザ）からの流量に対してレート制限を実施し、Core API への内部呼び出しにも独立した上限を持つことを MUST とする。
...
- **HTTP API 契約**
  BFF⇔Core API の API 契約は、スキーマ（例：OpenAPI/JSON Schema）と自動検証（CI での互換性チェック）を MUST とする。
