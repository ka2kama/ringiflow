...
型安全ステートマシン（[ADR-054](../70_ADR/054_型安全ステートマシンパターンの標準化.md)）により、無効な遷移は `Result::Err(DomainError::Validation)` を返す。型レベルでの防止ではなくランタイムのガードで制御している。
...
→ 詳細: [エンティティ影響マップ（WorkflowInstance）](../40_詳細設計書/エンティティ影響マップ/WorkflowInstance.md)
