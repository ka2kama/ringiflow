...
要件定義（[CORE-12 セクション 12.6](../10_要件定義書/01_コア要件.md#126-e2e-テストシナリオ)）の E2E テストシナリオと Playwright テスト（`tests/e2e/tests/`）の対応関係。
...
| 3 | E2E-004 | 承認フロー | 必須 | approval.spec.ts | 5 | カバー済み（1段階+2段階承認、確認ダイアログ表示・キャンセル） |
...
| 7 | E2E-007 | ファイルアップロード | 必須 | file-upload.spec.ts | 2 | カバー済み（ドキュメント管理アップロード、ワークフロー申請ファイル添付） |
| 8 | E2E-008 | 検索 | 推奨 | — | — | 未実装（機能未実装） |
| 9 | E2E-009 | ユーザー管理 | 推奨 | user-management.spec.ts | 4 | カバー済み（作成・編集・無効化、二重送信防止） |
...
|------|-----|
| 全シナリオ数 | 10 |
| カバー済み | 7 (15 テスト + 2 セットアップ) |
| 部分カバー | 1 (1 テスト) |
| 未実装（機能未実装） | 2 |
| 未実装（機能実装済み） | 0 |
| クリティカルパス カバー率 | 86%（6/7 必須シナリオ） |

注: カバレッジ目標はクリティカルパス 100%（CORE-12）。SSO は機能自体が未実装のため、機能実装時に E2E テストを追加する。
...
| 2026-02-27 | #940 で E2E-004 承認フロー（確認ダイアログ表示・キャンセル）、E2E-009 ユーザー管理（二重送信防止）を追加 |
| 2026-03-08 | #1085 で E2E-007 ファイルアップロード（ドキュメント管理アップロード、ワークフロー申請ファイル添付）を追加 |
