...
Epic #648（Observability 基盤）の Story #649 で、ログ初期化コードに以下の変更が加わった:

- 環境変数 `LOG_FORMAT` による JSON / Pretty 出力の切り替えロジック
- JSON 出力の構造化フィールド構成（timestamp, level, target, message, span.service）を3サービスで統一する必要性
- 後続 Story（#650 Request ID 伝播、#651 PII マスキング、#654 計装）の基盤として、ログ初期化コードの変更頻度が上がる見込み

これにより、初版で「起動コードは安定しており変更頻度が極めて低い」とした前提が成立しなくなった。

### 判断変更

クローン 2 について、選択肢 2-A（jscpd:ignore）から **選択肢 2-B 相当（共通関数を shared に抽出）** に変更する。

ただし、初版で指摘した 2-B の懸念（shared にアプリ層の責務を持ち込む問題）に対しては以下の設計で対処:

1. `observability` feature gate で依存をオプショナル化（`openapi` feature と同じパターン）
2. `init_tracing()` は設定に基づく初期化のみ。サービス固有のスパン設定は呼び出し元の責務
3. domain / infra クレートは feature を有効にしないため、不要な依存を引き込まない

### 帰結

- ログ初期化コードの重複が解消され、Observability 関連の変更が1箇所で済む
- shared に tracing / tracing-subscriber 依存が追加される（observability feature 有効時のみ）
- jscpd:ignore マーカーが不要になり、意図的重複の管理コストが削減される
...
| 日付 | 変更内容 |
|------|---------|
| 2026-02-18 | 補遺: クローン 2 の判断変更（#649 Observability 基盤による前提変更） |
| 2026-02-16 | 初版作成 |
