...
単一の Lightsail インスタンス（$7/月、1GB RAM）に全コンポーネントをデプロイ。Cloudflare Free プランで SSL 終端と CDN を提供。
...
1. **コスト最適**: 月額 $7 で全要件を満たせる（初回90日無料）
...
|------|---------|
| 2026-01-19 | 初版作成 |
| 2026-02-06 | OS を Ubuntu 24.04 LTS から AlmaLinux 9.4 へ変更（RHEL 系の学習効果を重視）。プランを $10（2GB）から $7（1GB）に変更（Rust の軽量さを活かす） |
