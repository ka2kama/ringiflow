...
| 観点 | 選択肢 1: 規約ベース | 選択肢 2: ADT ステートマシン | 選択肢 3: Opaque Module |
|------|---------------------|------------------------|----------------------|
...
選択肢 2: ADT ベースステートマシンをプロジェクト標準とし、選択肢 1 を補助として併用する。
...
4. ナレッジベースに ADT ステートマシンの実装ガイドを追加する
...
| 本 ADR の概念 | 既知手法 | 関係 |
|--------------|---------|------|
| ADT ベースステートマシン | Making Impossible States Impossible（Richard Feldman, 2016） | Elm コミュニティの確立パターン。本 ADR はこれをプロジェクト標準として明文化 |
| ADT ベースステートマシン | Typestate Pattern（Rust） | Rust での類似概念。ジェネリクスによる状態パラメータ化まではせず、enum による ADT を採用 |
