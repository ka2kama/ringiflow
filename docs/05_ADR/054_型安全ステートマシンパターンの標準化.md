...
### 選択肢 2: 型安全ステートマシン（ADT ベース）

状態ごとに異なる型を定義し、各状態で有効なフィールドだけを持たせる。ADT（代数的データ型）を主な実装手段として使用する。
...
| 観点 | 選択肢 1: 規約ベース | 選択肢 2: 型安全ステートマシン | 選択肢 3: Opaque Module |
|------|---------------------|--------------------------|----------------------|
...
選択肢 2: 型安全ステートマシンをプロジェクト標準とし、選択肢 1 を補助として併用する。主な実装手段として ADT（Elm の Custom Type、Rust の enum）を使用する。
...
判断の主眼は「不正な状態遷移が型レベルで防止されているか」であり、特定の実装手段（ADT、ジェネリクス、trait bounds 等）に依存しない。
...
型安全ステートマシンの目的を達成する手段は複数ある。場面に応じて最適な手段を選択する。

| 手段 | 適用場面 | 例 |
|------|---------|-----|
| ADT（enum / Custom Type） | 状態の種類が有限で列挙可能な場合（主な手段） | `WorkflowInstanceState`、`PageState` |
| ジェネリクス / Phantom Type | 状態をコンパイル時にパラメータ化する場合 | Typestate Pattern |
| Trait bounds / 型クラス | 状態ごとに異なる操作を型で制約する場合 | `impl Submittable for PendingState` |

現在のプロジェクトでは ADT が主な実装手段だが、将来的に他の手段が適切な場面があれば対等に検討する。
...
1. Designer.elm の Model をステートマシンにリファクタリングする（#793 の根本対策、別 Issue で管理）
2. ギャップ発見の「状態網羅漏れ」観点を拡張する（#795）
3. 他のページの Model で同様の構造がないか確認する（新規実装時に適用）
4. ナレッジベースに型安全ステートマシンの実装ガイドを追加する
...
| 本 ADR の概念 | 既知手法 | 関係 |
|--------------|---------|------|
| 型安全ステートマシン | Making Impossible States Impossible（Richard Feldman, 2016） | Elm コミュニティの確立パターン。本 ADR はこれをプロジェクト標準として明文化 |
| 型安全ステートマシン | Typestate Pattern（Rust） | Rust での類似概念。ジェネリクスによる状態パラメータ化も含む、より広い概念 |
...
| 日付 | 変更内容 |
|------|---------|
| 2026-02-24 | #855: 命名を「ADT ベースステートマシン」→「型安全ステートマシン」に変更。判断基準を手段非依存に修正し、「実装手段の選択」セクションを追加 |
| 2026-02-23 | 初版作成 |
