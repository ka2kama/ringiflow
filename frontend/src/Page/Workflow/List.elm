...
viewStatusDropdown currentFilter =
    ...
    span [ class "flex items-center gap-2" ]
        ...
        , select [ id "status-filter", onInput (statusFromFilterValue >> SetStatusFilter), class "rounded-lg border border-secondary-300 bg-white px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:border-primary-500" ]
            (List.map
                (\( maybeStatus, label_ ) ->
                    option
...
viewWorkflowTable zone workflows =
    table [ class "w-full" ]
        [ thead [ class "bg-secondary-50" ]
            [ tr []
                [ th [ class "px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-secondary-600" ] [ text "ID" ]
                , th [ class "px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-secondary-600" ] [ text "タイトル" ]
                , th [ class "px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-secondary-600" ] [ text "ステータス" ]
                , th [ class "px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-secondary-600" ] [ text "作成日" ]
                ]
            ]
