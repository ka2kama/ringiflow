...
1. 最初のコミット後にセッションログの[スキャフォールドを作成する](#スキャフォールド早期作成)
2. 判断ポイントが発生したら即座に判断ログセクションに追記する
3. `/wrap-up` 時に残りのセクション（概要、背景と目的、成果物、学んだこと等）を仕上げる

注意: 判断ログ以外のセクション（概要、成果物など）はセッション完了後に書く方が正確なため、wrap-up 時に記載する。

## スキャフォールド早期作成

セッションログは最初のコミット後にスキャフォールドとして早期作成し、実装中にリアルタイムで追記できるようにする。Draft PR の早期作成と同じ「Write-ahead」の原則に基づく: 振り返って書くより、起きた時に書く方が情報の忠実度が高い。
...
- 必須セクション（概要、背景と目的、実施内容、成果物、学んだこと）と判断ログを含める
- オプションセクション（設計上の判断、議論の経緯、発見した問題、次のステップ）は該当する場合に wrap-up 時に追加する
- スキャフォールド自体は独立コミットせず、次の実装コミットに含める

### wrap-up 時の要不要判断

スキャフォールドの早期作成は「書ける状態を確保する」ためのもの。wrap-up 時に、セッションログの作成条件（コード変更または設計判断があった）に該当しないと判断した場合はスキャフォールドを削除する。
