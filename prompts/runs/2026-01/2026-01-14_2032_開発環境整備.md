...
pretty_assertions を workspace に追加してテストで使用するよう依頼があった。

### リリースプロファイル設定

debuginfo を追加したいという要望があり、エラー発生時の原因調査に必要な情報を残す設定を検討した。strip について質問があり、strip = "none" を明示するよう指示があった。

### バージョン管理

Node.js のバージョンについて、elm 関連が問題なく動くバージョンにするよう依頼があった。また、問題ない範囲で各バージョンを最新にするよう指示があった。
