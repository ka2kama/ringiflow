...
> `mod.rs` は古い書き方なので使わないでください。

→ Rust 2018 以降の新しいモジュール構造を採用し、ルールに明記した。

> async-trait を使った方がいいですか？

→ デファクトスタンダードかつベストプラクティスとして async-trait を採用。

> Issue の進捗管理ができていません。都度更新すべきです。

→ Phase 完了時に Issue を更新するルールを CLAUDE.md と手順書に追加。

> ルールの追加だけでもコミットを分けるべきですか？

→ 関心事の分離のため、別 PR でマージする方針に。
