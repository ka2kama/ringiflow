...
BFF と Core API を分けたはずだが古い API が残っているという指摘があり、調査を行った。`apps/api/` に古いコードが残存していることを確認し、削除を実施した。

マイグレーションファイルが消えていることについて確認があり、`apps/core-api/migrations/` に移動済みであることを説明した。
