...
ユーザーから、commit 時に lint してフォーマットも含めチェックしているはずだが、hook が動いていないのか間違っているのかという疑問が寄せられた。調査の結果、PostToolUse hook のコマンドに問題があることが判明した。

### ドキュメント化の要請

hook のコマンドが複雑なので、技術ノートかコメントで補足してほしいという要望があった。また、セッションログの作成も依頼された。これを受けて、技術ノートに環境変数とプロジェクト設定例を追加した。
