...
| `.lefthook/prepare-commit-msg/add-issue-number` | Issue 番号自動付与スクリプト |
| `docs/70_ADR/014_lefthook導入.md` | lefthook 導入の ADR |
...
| `docs/60_手順書/04_開発フロー/01_Issue駆動開発.md` | 規約と自動付与の説明追加 |
| `docs/60_手順書/01_開発参画/01_開発環境構築.md` | lefthook 追加、各ツールの説明と公式リンク追加 |
...
ユーザーから、コミットや PR が特定の Issue と紐付いている場合、コミットメッセージの先頭や PR のタイトルに Issue 番号を含めたいという要望があった。

### 自動化の検討

フックで強制することはできるか、ブランチ名の先頭に合わせるなどの方法はどうかという議論があった。これを受けて lefthook による自動付与を導入した。

### ツール管理の方針

mise で入れられるツールは mise で入れたいという要望があった。また、各ツールに公式へのリンクが欲しい、各ツールの簡単な説明も（1〜2行）欲しいという要望があり、手順書を更新した。

### lefthook の信頼性

lefthook は信頼できるツールかという確認があった。Evil Martians が開発し、GitLab でも採用されている信頼性の高いツールであることを確認した上で導入を決定した。
