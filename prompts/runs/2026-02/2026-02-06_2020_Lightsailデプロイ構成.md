...
AWS Lightsail + Cloudflare による個人検証環境を構築した。OS を AlmaLinux 9.4、プランを $5（1GB）の IPv6-only で構成し、実際のデプロイまで完了した。
...
2. プランを $10（2GB）から $5（1GB）に変更（Rust の軽量さを活かす）
...
| プラン | $5（1GB） | Rust サービスは軽量（3サービス合計 ~90MB）で 1GB に収まる |
...
| `5784b68` | プランを $10（2GB）から $5（1GB）に変更 |
...
- `docs/70_ADR/030_Lightsail個人環境の構築.md` — 変更履歴追記
