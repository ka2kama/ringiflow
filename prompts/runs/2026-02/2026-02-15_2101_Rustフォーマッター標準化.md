...
### 7. レビュー対応（wrap_comments / normalize_comments の除外）

**問題の発覚:**
- Claude Auto Review で `wrap_comments` と `normalize_comments` による複数の問題が検出
  - [High] OpenAPI 仕様の破損: doc コメントの改行が `summary` フィールドに混入
  - [Critical] Markdown リンクの破壊: 約15箇所でリンクが改行で分断
  - [Low] 日本語の不自然な折り返し: 単語の途中や不自然な位置で改行

**対応:**
1. `.rustfmt.toml` から `wrap_comments` と `normalize_comments` を削除
2. `cargo +nightly fmt` で全ファイルを再フォーマット（元の状態に戻った）
3. `just openapi-generate` で OpenAPI 仕様を再生成
4. レビューコメントに返信してスレッドを resolve
5. ADR-048 とナレッジベースに除外理由を追記

**結果:**
- OpenAPI 仕様と Markdown リンクが修復
- 最終的な unstable 設定: 14個（当初16個から2個除外）
...
### 判断 6: コメント自動整形の除外（判断2の撤回）

**背景**: Claude Auto Review で `wrap_comments` と `normalize_comments` による複数の問題が検出された。

**問題の内容**:
1. **OpenAPI 仕様の破損** [High]: utoipa は doc コメントを OpenAPI の `summary` フィールドに変換するが、`wrap_comments` による改行がそのまま仕様に混入し、API 仕様として不適切
2. **Markdown リンクの破壊** [Critical]: doc コメント内の Markdown リンクが改行で分断され、レンダリングが壊れる（約15箇所）
3. **日本語処理の限界** [Low]: 日本語の自然な区切り位置を判定できず、単語の途中や不自然な位置で改行

**決定**: `wrap_comments` と `normalize_comments` を `.rustfmt.toml` から除外

**理由**:
- OpenAPI 生成（utoipa）との非互換性が致命的
- Markdown ドキュメントとの非互換性が広範囲
- 日本語コメントが主体のプロジェクトでは不自然な折り返しが頻発
- 自動折り返しは魅力的だが、プロジェクトの文脈では有害と判断

**トレードオフ**:
- コメントの折り返しは手動管理に戻る
- ただし、より重要な OpenAPI 仕様の正確性と Markdown の可読性を優先
...
- **ナレッジベース**: `docs/80_ナレッジベース/rust/rustfmt.md`（設定カテゴリテーブルを追加）
