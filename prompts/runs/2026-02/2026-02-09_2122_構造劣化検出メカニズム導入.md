...
1. `fe3c88c` - Introduce cargo-machete for unused dependency detection
2. `c5a12f5` - Add file size threshold alert for structural degradation detection
3. `134bf59` - Add structural review guidelines and integrate into quality checklist
4. `8c40839` - Add session log and rename plan file for #151

### 作成・更新ファイル

| ファイル | 操作 |
|---------|------|
| `docs/05_ADR/038_未使用依存検出ツールの選定.md` | 新規 |
| `scripts/check-file-size.sh` | 新規 |
| `.claude/rules/structural-review.md` | 新規 |
| `justfile` | 更新（check-unused-deps、check-file-size 追加） |
| `.github/workflows/ci.yaml` | 更新（cargo-machete ステップ追加） |
| `CLAUDE.md` | 更新（構造品質維持セクション追加） |
| `docs/04_手順書/01_開発参画/01_開発環境構築.md` | 更新（cargo-machete セクション追加） |
| `docs/04_手順書/04_開発フロー/01_Issue駆動開発.md` | 更新（構造的健全性チェック追加） |
...
- PR #351 を Ready for Review にする
- CI で cargo-machete ステップが正常に動作することを確認する
