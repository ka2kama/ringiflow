...
- コンテキスト依存エラーマッピング: `get_task_by_display_numbers` ハンドラでは `StepNotFound` を `task-not-found` にマッピングする必要があった（API 契約の維持）。集約された `IntoResponse` では `step-not-found` になるため、ハンドラ固有の `map_err` で上書き

## 成果物

コミット:

- `66d3906` — `#526 Reduce BFF handler boilerplate with authenticate helper and centralized error mapping`
- `cc6f2d8` — `#526 Fix task-not-found error type in get_task_by_display_numbers handler`
