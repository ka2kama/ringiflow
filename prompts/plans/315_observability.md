...
### 記録のタイミング: リアルタイム（実装中に追記する）

当初は wrap-up 時にまとめて記載する設計だったが、ユーザーとの議論で修正。

問題: wrap-up 時の再構成では、判断時のコンテキスト（検討した選択肢、棄却理由、違和感の正体）が失われる。コミットメッセージも squash merge で消えるため、手がかりにできない。

修正後: 判断ログはセッション中にリアルタイムで記録する。セッションログファイルは最初の判断ポイントで作成し、以降は都度追記する。残りのセクション（概要、成果物等）は wrap-up 時に仕上げる。
...
新規ファイル: `docs/80_ナレッジベース/methodology/AI思考特性の分析ガイド.md`
