...
|------------|------|---------------|------|
| 10_要件定義書 | 連番 | `00_はじめに.md` | OK |
| 20_機能仕様書 | 連番 | `00_はじめに.md` | OK |
| 30_基本設計書 | 連番 | なし（`00_アーキテクチャ概要.md` が先頭） | GAP |
| 40_詳細設計書 | 連番 | なし（`00_実装ロードマップ.md` が先頭） | GAP |
| 60_手順書 | 連番 | `00_はじめに.md` | OK |
| 70_ADR | コレクション | `README.md` | OK |
| 80_ナレッジベース | コレクション | `README.md` | OK |
| 90_実装解説 | コレクション | `README.md` | OK |
| 50_テスト | 未成熟 | なし（ファイル1つのみ） | GAP |
| 40_詳細設計書/エンティティ影響マップ | コレクション | `README.md` | OK |
...
実態:
- コレクション型ディレクトリ（70_ADR, 80_ナレッジベース, 90_実装解説）のファイルは連番プレフィックスを使っていない
  - 70_ADR: `001_`, `002_` 形式（3桁連番、docs 標準の `00_` 形式ではない）
  - 80_ナレッジベース: サブディレクトリ + 自由命名
  - 90_実装解説: サブディレクトリ（一部連番）+ 自由命名
...
| ディレクトリ | 判断 | 理由 |
|------------|------|------|
| 30_基本設計書 | 本 PR では見送り、#323 で対応 | `00_アーキテクチャ概要.md` が既に `00_` スロットを占有。追加にはリナンバリングが必要 |
| 40_詳細設計書 | 本 PR では見送り、#323 で対応 | `00_実装ロードマップ.md` が既に `00_` スロットを占有。同上 |
| 50_テスト | 本 PR では見送り | ファイルが1つしかなく、ディレクトリ構造が未成熟。エントリファイルを作るにはまず体系が必要 |
...
- `30_基本設計書`: `00_アーキテクチャ概要.md` が `00_` スロットを占有 → リナンバリングが必要
- `40_詳細設計書`: `00_実装ロードマップ.md` が `00_` スロットを占有 → 同上
- `50_テスト`: ファイルが1つのみ、ディレクトリ構造が未成熟
