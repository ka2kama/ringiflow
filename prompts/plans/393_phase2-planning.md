...
| ドキュメント | 確認内容 |
|-------------|---------|
| `docs/10_要件定義書/01_コア要件.md` | CORE-03, WF-002, WF-103, TASK-005, NOTIFY-001, DOC-001, DOC-003, ADM-002, ADM-003, RPT-004 の定義 |
| `docs/30_基本設計書/03_インフラとDB設計.md` | RLS 二重防御設計（7.1.3 節）の詳細度 |
| `docs/40_詳細設計書/06_テナント退会時データ削除設計.md` | Phase 2 追加データストア（監査ログ、S3）との整合性 |
| `docs/20_機能仕様書/01_ワークフロー管理.md` | 複数ステップ承認・差し戻しのシナリオ |

不足があれば Issue 化して記録する（本 PR では修正しない）。

### Phase 3: 実装ロードマップ更新

`docs/40_詳細設計書/00_実装ロードマップ.md` の Phase 2 セクションを更新する。
...
- 型: なし（ドキュメント作業）
- パターン: 既存ロードマップの Phase 0-1 セクションの記述パターン → `docs/40_詳細設計書/00_実装ロードマップ.md`
- ライブラリ: なし
