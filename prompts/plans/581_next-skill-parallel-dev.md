...
## Phase 1: SKILL.md 書き換え

### 確認事項
- [x] パターン: 現行の SKILL.md 構造 → `.claude/skills/next/SKILL.md` を Read で確認済み
- [x] パターン: worktree コマンドの使用法 → `git worktree list --porcelain` の出力形式を確認済み
- [x] パターン: 既存の just コマンド → `just worktree-issue` が利用可能であることを確認済み

### テストリスト

ユニットテスト（該当なし — スキル定義ファイルであり実行可能コードではない）
...
手動検証:
- [ ] `/next` を実行し、進行中の worktree が「進行中の作業」セクションに表示されること
- [ ] 進行中の Issue が候補リストに含まれないこと
- [ ] `type:epic` と `idea` ラベルの Issue が候補に含まれないこと
- [ ] カテゴリが異なる Issue が「並行着手が可能な候補」に表示されること
- [ ] 同カテゴリの Issue が「他の候補」に表示されること
- [ ] 進行中の作業がない場合、従来通りフラット表示になること
...
| ループ | 検出したギャップ | 観点 | 対応 |
|-------|----------------|------|------|
| 1回目 | 現行スキルに worktree 認識がなく、別 worktree の作業中 Issue が候補に出る | 未定義 | Step 1 に `git worktree list --porcelain` を追加し、全 worktree からの Issue 収集を設計 |
| 2回目 | Epic/idea が候補に混入する問題が未対処 | 不完全なパス | Step 3 のフィルタリングに `type:epic` と `idea` ラベルの除外条件を追加 |
| 3回目 | 並行可能な候補の判断基準が依存関係のみでは不十分 | 既存手段の見落とし | カテゴリラベル（backend/frontend 等）によるグルーピングを Step 5 として新設 |

## 収束確認（設計・計画）

| # | 観点 | 理想状態（To-Be） | 判定 | 確認内容 |
|---|------|------------------|------|---------|
| 1 | 網羅性 | 探索で発見された全対象が計画に含まれている | OK | 現行 SKILL.md の全 Step を確認し、改修後の 7 Step で全機能をカバー。worktree 認識、フィルタリング、グルーピング、出力形式、着手案内を網羅 |
| 2 | 曖昧さ排除 | 不確定な記述がゼロ | OK | 各 Step の条件・出力形式が具体的に定義されている。「必要に応じて」等の不確定表現なし |
| 3 | 設計判断の完結性 | 全ての差異に判断が記載されている | OK | 4つの設計判断（進行中で停止しない、カテゴリベース、worktree 優先、Epic/idea 除外）に理由を記載 |
| 4 | スコープ境界 | 対象と対象外が両方明記されている | OK | 対象: SKILL.md、対象外: /restore スキル、worktree スクリプト群を明記 |
| 5 | 技術的前提 | コードに現れない前提が考慮されている | OK | `git worktree list --porcelain` の出力形式、`just worktree-issue` の存在を確認済み |
| 6 | 既存ドキュメント整合 | 既存ドキュメントと矛盾がない | OK | ADR-021（並行開発環境）と整合。`/restore` スキルとの連携を維持 |
