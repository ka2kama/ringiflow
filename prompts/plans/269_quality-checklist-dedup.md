...
品質チェックリストの具体的な確認項目は [Issue駆動開発 > 品質チェックリスト](../../docs/60_手順書/04_開発フロー/01_Issue駆動開発.md#62-品質チェックリスト) を参照。
...
```markdown
実装中は [TDD の Refactor ステップ](../../docs/60_手順書/04_開発フロー/02_TDD開発フロー.md#-refactor-きれいにする) が俯瞰のトリガーとなり、[品質チェックリスト](../../docs/60_手順書/04_開発フロー/01_Issue駆動開発.md#62-品質チェックリスト)で品質を維持する。特定コンテキストの詳細ルール（[api.md](../../.claude/rules/api.md), [repository.md](../../.claude/rules/repository.md), [data-store.md](../../.claude/rules/data-store.md)）は俯瞰時の観点を提供する。
```
