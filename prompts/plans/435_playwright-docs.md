...
| # | ファイル | 操作 |
|---|---------|------|
| 1 | `docs/80_ナレッジベース/devtools/Playwright.md` | 新規 |
| 2 | `docs/30_基本設計書/01_アーキテクチャ設計.md` | 追記（テスト戦略セクション） |
| 3 | `docs/50_テスト/E2Eテスト突合表.md` | 新規 |
| 4 | `.claude/rules/e2e-test.md` | 新規 |
| 5 | `docs/60_手順書/04_開発フロー/02_TDD開発フロー.md` | 追記（E2E テスト実行タイミング） |
...
### 確認事項
- パターン: 既存ナレッジベース `docs/80_ナレッジベース/devtools/hurl.md` の構成を踏襲
...
### 確認事項
- パターン: `docs/30_基本設計書/01_アーキテクチャ設計.md` の既存構造 → Read で確認
- 型: テストピラミッドの定義 → `docs/10_要件定義書/01_コア要件.md` CORE-12（L1343-1498）
- パターン: ADR-032 のレイヤーごとのテスト表現形式

### 内容

`docs/30_基本設計書/01_アーキテクチャ設計.md` に「テスト戦略」セクションを追加する。分散している情報を基本設計レベルで統合する:
...
### 確認事項
- パターン: `docs/50_テスト/APIテスト突合表.md` の構成を踏襲
- 型: E2E テストシナリオ一覧 → `docs/10_要件定義書/01_コア要件.md` CORE-12 セクション 12.6（L1460-1473）
...
### 確認事項
- パターン: `docs/60_手順書/04_開発フロー/02_TDD開発フロー.md` の既存構造
- 型: 変更履歴テーブルの形式
...
| 2回目 | 実装解説ドキュメント（`docs/90_実装解説/18_E2Eテスト/`）が #128 で既に作成済みで、ナレッジベースとの住み分けが必要 | 責務の明確さ | ナレッジベースはツールの汎用的な使い方、実装解説は PR 固有の設計判断と位置づけ。重複を避ける |
